...
function EquipmentForm({ edit = false }: { edit?: boolean }) {
  ...
  return <Page title={edit ? "编辑设备" : "新增设备"}>
    <form className="equipment-form" onSubmit={submit}>
      ...
      <fieldset><legend>基础身份</legend><div className="portal-form"><label>设备编码<input name="code" required defaultValue={current?.code} /></label><label>设备名称<input name="name" required defaultValue={current?.name} /></label><label>型号<input name="model" required defaultValue={current?.model} /></label><label>类型<input name="type" required defaultValue={current?.type} /></label><label>制造商<input name="manufacturer" required defaultValue={current?.manufacturer} /></label><label>所属产线<select name="organization_id" aria-label="所属产线" required defaultValue={current?.organization_id ?? lines[0]?.id ?? ""}><option value="" disabled>请选择启用产线</option>{lines.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label></div></fieldset>
      ...
      <fieldset className="equipment-boundary-section"><legend>设备 BOM 组成</legend><div className="section-title"><button type="button" className="button-secondary" disabled>新增分支节点</button></div><p>当前接口未提供该设备的 BOM、额定参数或知识资料数据。</p></fieldset>
      <fieldset className="equipment-boundary-section"><legend>设备额定参数</legend><div className="section-title"><button type="button" className="button-secondary" disabled>新增参数</button></div><p>当前接口未提供该设备的额定参数数据。</p></fieldset>
      <fieldset className="equipment-boundary-section"><legend>知识资料</legend><div className="section-title"><button type="button" className="button-secondary" disabled>上传资料</button></div><p>当前接口未提供该设备的知识资料数据。</p></fieldset>
      <footer><button type="submit" disabled={!lines.length || saving}>{saving ? "保存中…" : "保存"}</button>{!lines.length && <p role="alert">没有可用产线，无法保存设备。</p>}{message && <p role="status">{message}</p>}</footer>
    </form>
  </Page>;
}
