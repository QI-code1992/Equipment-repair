...
function EquipmentForm({ edit = false }: { edit?: boolean }) {
  ...
  const organizationItems = organizations.value ?? [];
  const selectedLineId = lineId || (edit ? current?.organization_id ?? "" : "");
  const selectedLine = organizationItems.find((item) => item.id === selectedLineId);
  const selectedWorkshopId = workshopId || selectedLine?.parent_id || "";
  const selectedWorkshop = organizationItems.find((item) => item.id === selectedWorkshopId);
  const selectedFactoryId = factoryId || selectedWorkshop?.parent_id || "";
  const factories = organizationItems.filter((item) => item.type === "FACTORY" && item.enabled);
  const workshops = organizationItems.filter((item) => item.type === "WORKSHOP" && item.enabled && item.parent_id === selectedFactoryId);
  const lines = organizationItems.filter((item) => item.type === "LINE" && item.enabled && item.parent_id === selectedWorkshopId);
  ...
  return <Page title={edit ? "编辑设备" : "新增设备"}>
    <form className="equipment-form" onSubmit={submit}>
      ...
      <fieldset><legend>设备基础信息</legend><div className="portal-form equipment-basic-grid"><label>设备编码<input name="code" required defaultValue={current?.code} /></label><label>设备名称<input name="name" required defaultValue={current?.name} /></label><label>型号<input name="model" required defaultValue={current?.model} /></label><label>类型<input name="type" required defaultValue={current?.type} /></label><label>制造商<input name="manufacturer" required defaultValue={current?.manufacturer} /></label><label>所属工厂<select aria-label="所属工厂" required value={selectedFactoryId} onChange={(event) => { setFactoryId(event.target.value); setWorkshopId(""); setLineId(""); }}><option value="">请选择工厂</option>{factories.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label>所属车间<select aria-label="所属车间" required disabled={!selectedFactoryId} value={selectedWorkshopId} onChange={(event) => { setWorkshopId(event.target.value); setLineId(""); }}><option value="">请选择车间</option>{workshops.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label>所属产线<select name="organization_id" aria-label="所属产线" required disabled={!selectedWorkshopId} value={selectedLineId} onChange={(event) => setLineId(event.target.value)}><option value="">请选择产线</option>{lines.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label></div></fieldset>
      ...
      <footer><Link className="button-secondary" to="/equipment">取消</Link><button type="submit" disabled={!selectedLineId || saving}>{saving ? "保存中…" : "保存设备"}</button>{!selectedLineId && <p role="alert">请选择启用的工厂、车间和产线后再保存。</p>}{message && <p role="status">{message}</p>}</footer>
    </form>
  </Page>;
}
