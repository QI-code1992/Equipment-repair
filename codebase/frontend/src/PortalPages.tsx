...
export function BiDashboardPage() {
  ...
  return <Page title="驾驶舱 BI">
    ...
    <State state={state}>{(value) => <>
      ...
      <section className="data-card bi-section" aria-labelledby="bi-trend-heading"><header className="section-heading"><div><h3 id="bi-trend-heading">趋势分析</h3><p>按时间观察健康、故障和工单变化。</p></div></header>{view === "trend" ? value.trend.length ? <TrendChart trend={value.trend} /> : <div className="prototype-unavailable"><p>暂无趋势数据。</p><p>当前 API 未提供趋势序列数据。</p></div> : value.organization_ranking.length ? <ol>{value.organization_ranking.map((item) => <li key={item.organization_id}>{item.organization_name}：{item.fault_count}</li>)}</ol> : <p className="prototype-unavailable">当前 API 未提供组织排行数据。</p>}</section>
...
function EquipmentForm({ edit = false }: { edit?: boolean }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const body = {
      ...
      organization_id: lineId || String(form.get("organization_id")),
