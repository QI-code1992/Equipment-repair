...
function RoleEditorModal({ mode, role, permissions, canWrite, onClose, onSaved, onFailed }: { mode: "new" | "edit"; role?: { id: string; name: string; permission_codes: string[] }; permissions: Array<{ code: string }>; canWrite: boolean; onClose: () => void; onSaved: () => void; onFailed: () => void }) {
  ...
  const [saving, setSaving] = useState(false);
  const permissionLabel = (code: string) => ({ workbench: "工作台", bi: "驾驶舱 BI", organization: "工厂建模", equipment: "设备台账", fault: "故障上报", maintenance: "维修记录", identity: "系统管理", intelligence: "智能配置", "workbench:view": "查看工作台", "bi:view": "查看驾驶舱", "bi:export": "导出 BI 记录", "organization:read": "查看组织", "organization:write": "维护组织", "equipment:read": "查看设备", "equipment:write": "维护设备", "fault:create": "提交故障", "fault:repair": "执行维修", "maintenance:view": "查看维修记录", "maintenance:detail": "查看维修详情", "identity:read": "查看系统管理", "identity:write": "维护用户和角色", "intelligence:agent": "使用 Agent", "intelligence:audit": "查看智能审计", "intelligence:knowledge": "维护知识库", "intelligence:model": "管理模型" } as Record<string, string>)[code] ?? code;
