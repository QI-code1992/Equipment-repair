...
export function BiDashboardPage() {
  ...
  const reset = () => { setOrganizationId(""); setPeriod("week"); setView("trend"); setNotice("筛选条件已重置。"); };
  return <Page title="驾驶舱 BI"><form className="bi-filter" aria-label="驾驶舱筛选条件" onSubmit={(event) => { event.preventDefault(); setNotice("已按当前筛选条件查询正式数据。"); }}><div className="bi-filter__grid"><label>时间范围<select aria-label="时间范围" value={period} onChange={(event) => setPeriod(event.target.value as typeof period)}><option value="day">最近 1 天</option><option value="week">最近 7 天</option><option value="month">最近 30 天</option></select></label><label>组织<select aria-label="组织筛选" value={organizationId} onChange={(event) => setOrganizationId(event.target.value)} disabled={organizations.loading || !!organizations.error}><option value="">全部组织</option>{organizations.value?.filter((item) => item.enabled).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label>区域<select aria-label="区域筛选" disabled title="当前 BI API 未提供区域筛选字段"><option>全部区域</option></select></label><label>设备类型<select aria-label="设备类型筛选" disabled title="当前 BI API 未提供设备类型筛选字段"><option>全部类型</option></select></label></div><div className="bi-filter__actions"><button className="button-primary" type="submit">查询</button><button className="button-secondary" type="button" onClick={reset}>重置</button></div></form>{notice && <p role="status" className="bi-notice">{notice}</p>}{organizations.error && <p role="alert">组织筛选不可用：{organizations.error}</p>}{state.error?.includes("ORGANIZATION_NOT_FOUND") && <p role="alert">所选组织不存在或已被移除。</p>}<State state={state}>{(value) => <>
    ...
    <section className="data-card bi-analytics-panel" aria-labelledby="bi-trend-title"><div className="section-title"><div><h3 id="bi-trend-title">趋势分析</h3><p>按时间观察健康、故障和工单的变化趋势</p></div><div className="bi-segmented" role="group" aria-label="趋势视图"><button type="button" className={view === "trend" ? "is-active" : ""} onClick={() => setView("trend")}>趋势</button><button type="button" className={view === "ranking" ? "is-active" : ""} onClick={() => setView("ranking")}>组织排行</button></div></div>{view === "trend" ? <div className="bi-trend-grid"><section className="bi-chart-panel"><h4>健康综合评分趋势</h4><p className="empty-panel">当前 BI API 未提供健康评分趋势。</p></section><section className="bi-chart-panel"><h4>故障数量趋势</h4>{value.trend.length ? <TrendChart trend={value.trend} /> : <p>暂无趋势数据。</p>}</section><section className="bi-chart-panel"><h4>工单数量趋势</h4>{value.trend.length ? <BiOrderTrendChart trend={value.trend} /> : <p>暂无趋势数据。</p>}</section></div> : <section className="bi-ranking-panel"><h4>组织故障排行</h4>{value.organization_ranking.length ? <ol>{value.organization_ranking.map((item) => <li key={item.organization_id}>{item.organization_name}<strong>{item.fault_count}</strong></li>)}</ol> : <p className="empty-panel">暂无排行数据。</p>}</section>}</section>
    <section className="data-card bi-analytics-panel" aria-labelledby="bi-efficiency-title"><div className="section-title"><div><h3 id="bi-efficiency-title">效率分析</h3><p>聚合指标用于管理层效率判断，不展示具体工单列表</p></div></div><div className="bi-efficiency-grid"><BiMetric label="当前窗口完成工单" value={`${value.efficiency.completed_work_order_count}`} suffix="单" /><BiMetric label="平均完成时长" value={value.efficiency.average_completion_hours === null ? "—" : `${value.efficiency.average_completion_hours}`} suffix={value.efficiency.average_completion_hours === null ? undefined : "小时"} unavailable={value.efficiency.average_completion_hours === null} /><BiMetric label="活动故障" value={`${value.summary.active_fault_count}`} suffix="项" /></div></section>
    <section className="data-card bi-health-list" aria-labelledby="bi-health-list-title"><div className="section-title"><div><h3 id="bi-health-list-title">设备健康列表</h3><p>健康评分和风险等级需由设备健康聚合接口提供。</p></div><a className="button-secondary" href="/equipment">查看全部设备</a></div><p className="empty-panel">当前 BI API 未提供设备健康列表及健康评分字段。</p></section>
...
function SystemManagementContent({ canWrite }: { canWrite: boolean }) {
  ...
  const roleItems = (roles.value ?? []).filter((item) => !roleQuery || [item.name, item.code].some((value) => value.toLowerCase().includes(roleQuery.toLowerCase())));
  const userItems = (users.value ?? []).filter((item) => !userQuery || item.username.toLowerCase().includes(userQuery.toLowerCase()));
  const enabledUsers = (users.value ?? []).filter((item) => item.enabled).length;
  return <Page title="系统管理">
    <section className="system-overview" aria-label="系统管理概览"><div><p className="page-shell__eyebrow">统一身份与审计</p><h3>系统角色、组织与用户统一维护</h3><p>所有身份、权限和日志数据均来自正式服务端接口。</p></div><div className="system-overview__stats"><span>角色总数<strong>{roles.value?.length ?? "—"}</strong></span><span>系统用户<strong>{users.value?.length ?? "—"}</strong></span><span>启用账号<strong>{users.value ? enabledUsers : "—"}</strong></span></div></section>
    ...
    <section className="section-tab-panel system-tab-panel" role="tabpanel">
      {section === "roles" && <><div className="system-toolbar"><label>搜索角色<input aria-label="搜索角色名称" value={roleQuery} onChange={(event) => setRoleQuery(event.target.value)} placeholder="搜索角色名称" /></label><div><button type="button" className="button-primary" disabled title="当前公开 API 未提供角色创建">新增角色</button><button type="button" className="button-secondary" onClick={() => setRefresh((value) => value + 1)}>刷新</button></div></div><div className="panel-heading"><div><h3>角色列表</h3><p>角色与权限编码由正式身份接口返回。</p></div><span className="status-chip status-chip--success">{roleItems.length} 个角色</span></div><State state={roles} empty={(items) => !items.length}>{() => <>{roleItems.length ? <table><thead><tr><th>角色名称</th><th>角色编码</th><th>权限数量</th><th>状态</th><th>操作</th></tr></thead><tbody>{roleItems.map((role) => <tr key={role.id}><td>{role.name}</td><td>{role.code}</td><td>{role.permission_codes.length}</td><td>正式接口未提供</td><td>{canWrite ? <button type="button" className="link-button" onClick={() => setEditingRoleId((current) => current === role.id ? null : role.id)}>{editingRoleId === role.id ? "收起权限" : "编辑权限"}</button> : "只读"}</td></tr>)}</tbody></table> : <p className="empty-panel">没有符合条件的正式角色。</p>}{editingRoleId && roleItems.find((item) => item.id === editingRoleId) && <RolePermissionEditor role={roleItems.find((item) => item.id === editingRoleId)!} permissions={permissions.value ?? []} canWrite={canWrite} onSaved={() => { setRefresh((value) => value + 1); setNotice("角色权限已更新。"); }} onFailed={() => setNotice("角色更新失败，请检查权限后重试。")} />}<section className="system-permission-catalogue"><h4>权限配置目录</h4><State state={permissions} empty={(items) => !items.length}>{(items) => <div className="permission-catalogue">{items.map((item) => <code key={item.code}>{item.code}</code>)}</div>}</State></section></>}</State></>}
      {section === "users" && <><div className="system-users-layout"><aside className="system-org-boundary"><div className="panel-heading"><div><h3>组织树</h3><p>用于按组织筛选系统用户。</p></div></div><p className="empty-panel">当前身份管理接口未提供组织树关联数据。</p><button type="button" className="button-primary" disabled>新增组织</button><button type="button" className="button-secondary" disabled>删除组织</button></aside><section className="system-users-panel"><div className="system-toolbar"><label>搜索用户<input aria-label="搜索用户名" value={userQuery} onChange={(event) => setUserQuery(event.target.value)} placeholder="搜索用户名" /></label><div><button type="button" className="button-secondary" onClick={() => setRefresh((value) => value + 1)}>刷新</button></div></div><div className="panel-heading"><div><h3>用户列表</h3><p>仅身份写入权限可新增或调整账号状态。</p></div><span className={`status-chip ${canWrite ? "status-chip--success" : "status-chip--neutral"}`}>{canWrite ? "可管理" : "只读"}</span></div><State state={users} empty={(items) => !items.length}>{() => <>{userItems.length ? <table><thead><tr><th>账号</th><th>状态</th><th>角色</th><th>操作</th></tr></thead><tbody>{userItems.map((item) => <tr key={item.id}><td>{item.username}</td><td>{item.enabled ? "启用" : "停用"}</td><td>{item.role_ids.join(", ") || "未分配"}</td><td>{canWrite ? <button type="button" disabled={saving} onClick={() => void toggle(item)}>{item.enabled ? "停用" : "启用"}</button> : "只读"}</td></tr>)}</tbody></table> : <p className="empty-panel">没有符合条件的正式用户。</p>}{canWrite && <form className="portal-form" onSubmit={create}><label>用户名<input name="username" required /></label><label>初始密码<input name="password" type="password" minLength={8} required /></label><label>角色<select name="role_id">{roles.value?.map((role) => <option key={role.id} value={role.id}>{role.name}</option>)}</select></label><button type="submit" disabled={saving}>创建账号</button></form>}</>}</State></section></div></>}
      {section === "loginLogs" && <AuditLogPanel title="登录日志" description="仅展示审计接口中动作包含 login 的记录。" data={audits} page={auditPage} action={auditAction} onActionChange={(value) => { setAuditAction(value); setAuditPage(1); }} onPageChange={setAuditPage} predicate={(item) => item.action.toLowerCase().includes("login")} />}
      {section === "operationLogs" && <AuditLogPanel title="操作日志" description="展示服务端保留的正式审计事件。" data={audits} page={auditPage} action={auditAction} onActionChange={(value) => { setAuditAction(value); setAuditPage(1); }} onPageChange={setAuditPage} />}
    </section>
  </Page>;
}

function AuditLogPanel({ title, description, data, page, action, onActionChange, onPageChange, predicate = () => true }: { title: string; description: string; data: LoadState<{ items: AuditEvent[]; count: number; page: number; page_size: number }>; page: number; action: string; onActionChange: (value: string) => void; onPageChange: React.Dispatch<React.SetStateAction<number>>; predicate?: (item: AuditEvent) => boolean }) { return <><div className="system-toolbar"><label>日志搜索<input aria-label={`${title}动作筛选`} value={action} onChange={(event) => onActionChange(event.target.value)} placeholder="输入正式动作" /></label></div><div className="panel-heading"><div><h3>{title}</h3><p>{description}</p></div></div><State state={data} empty={(value) => !value.count}>{(value) => { const items = value.items.filter(predicate); return items.length ? <><table><thead><tr><th>时间</th><th>动作</th><th>资源</th><th>结果</th></tr></thead><tbody>{items.map((item) => <tr key={item.id}><td>{item.created_at}</td><td>{item.action}</td><td>{item.resource_type}</td><td>{item.result}</td></tr>)}</tbody></table><div className="pager"><button type="button" disabled={page <= 1} onClick={() => onPageChange((current) => current - 1)}>上一页</button><span>第 {page} 页</span><button type="button" disabled={value.items.length < value.page_size} onClick={() => onPageChange((current) => current + 1)}>下一页</button></div></> : <p className="empty-panel">暂无符合条件的正式{title}。</p>; }}</State></>; }
