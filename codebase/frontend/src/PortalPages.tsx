...
export function BiDashboardPage() {
  ...
  const state = useData<BiDashboard>(() => getBiDashboard(organizationId || undefined), [organizationId]);
  const organizations = useData(getOrganizations, []);
  return <Page title="驾驶舱 BI"><label>组织筛选<select aria-label="组织筛选" value={organizationId} onChange={(event) => setOrganizationId(event.target.value)} disabled={organizations.loading || !!organizations.error}><option value="">全部组织</option>{organizations.value?.filter((item) => item.enabled).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>{organizations.error && <p role="alert">组织筛选不可用：{organizations.error}</p>}<State state={state}>{(value) => <div className="data-grid"><div className="metric"><b>{value.summary.fault_count}</b><span>故障总数</span></div><div className="metric"><b>{value.summary.active_fault_count}</b><span>活动故障</span></div><div className="metric"><b>{value.summary.completed_work_order_count}</b><span>完成工单</span></div><div className="metric"><b>{Math.round(value.summary.completion_rate * 100)}%</b><span>完成率</span></div><section className="data-card"><h3>近 7 日趋势</h3>{value.trend.length ? <ul>{value.trend.map((item) => <li key={item.date}>{item.date}：故障 {item.fault_count}，完成 {item.completed_work_order_count}</li>)}</ul> : <p>暂无趋势数据。</p>}</section><section className="data-card"><h3>效率与历史对比</h3><p>{value.efficiency.average_completion_hours === null ? "暂无具有开始与完成时间的工单，无法计算平均完成时长。" : `平均完成 ${value.efficiency.average_completion_hours} 小时`}</p><p>近 7 日故障 {value.history_comparison.current_fault_count}，上个 7 日故障 {value.history_comparison.previous_fault_count}。</p></section><section className="data-card"><h3>组织故障排行</h3>{value.organization_ranking.length ? <ol>{value.organization_ranking.map((item) => <li key={item.organization_id}>{item.organization_name}：{item.fault_count}</li>)}</ol> : <p>暂无排行数据。</p>}</section></div>}</State></Page>;
}
