...
export function BiDashboardPage() {
  ...
  return <Page title="驾驶舱 BI">
    ...
    <State state={state}>{(value) => <>
      <section className="bi-section" aria-labelledby="bi-summary-heading"><header className="section-heading"><div><h3 id="bi-summary-heading">管理摘要</h3><p>整体经营与运维表现，数据来自正式 BI 接口。</p></div></header><div className="data-grid"><div className="metric"><b>{value.summary.fault_count}</b><span>故障总数</span></div><div className="metric"><b>{value.summary.active_fault_count}</b><span>活动故障</span></div><div className="metric"><b>{value.summary.completed_work_order_count}</b><span>完成工单</span></div><div className="metric"><b>{Math.round(value.summary.completion_rate * 100)}%</b><span>完成率</span></div><div className="metric metric--warning"><b>{value.efficiency.average_completion_hours ?? "—"}</b><span>平均维修时长（小时）</span></div></div></section>
      <section className="data-card bi-section" aria-labelledby="bi-trend-heading"><header className="section-heading"><div><h3 id="bi-trend-heading">趋势分析</h3><p>按时间观察健康、故障和工单变化。</p></div></header>{view === "trend" ? value.trend.length ? <><div className="bi-trend-grid"><article className="data-card"><h3>健康综合评分趋势</h3><p className="prototype-unavailable">当前 BI API 未提供健康评分序列。</p></article><article className="data-card"><h3>故障数量趋势</h3><TrendSeriesChart label="故障数量" values={value.trend.map((item) => item.fault_count)} dates={value.trend.map((item) => item.date)} /></article><article className="data-card"><h3>工单数量趋势</h3><TrendSeriesChart label="工单数量" values={value.trend.map((item) => item.completed_work_order_count)} dates={value.trend.map((item) => item.date)} /></article></div><TrendChart trend={value.trend} /></> : <div className="prototype-unavailable"><p>暂无趋势数据。</p><p>当前 API 未提供趋势序列数据。</p></div> : value.organization_ranking.length ? <ol>{value.organization_ranking.map((item) => <li key={item.organization_id}>{item.organization_name}：{item.fault_count}</li>)}</ol> : <p className="prototype-unavailable">当前 API 未提供组织排行数据。</p>}</section>
      <section className="data-card bi-section" aria-labelledby="bi-efficiency-heading"><header className="section-heading"><div><h3 id="bi-efficiency-heading">效率分析</h3><p>聚合指标用于管理层效率判断。</p></div></header><div className="bi-unavailable-grid"><p>计划工单完成率：当前 API 未提供。</p><p>平均响应时长：当前 API 未提供。</p><p>{value.efficiency.average_completion_hours === null ? "平均完成时长：当前 API 未提供。" : `平均完成 ${value.efficiency.average_completion_hours} 小时`}</p><p>首次修复率：当前 API 未提供。</p></div></section>
...
function EquipmentForm({ edit = false }: { edit?: boolean }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const body = {
      ...
      organization_id: lineId || current?.organization_id || String(form.get("organization_id")),
...
export function AgentReportPage() {
  ...
  return <Page title="AI 故障上报">
    ...
    <section className="agent-report-module-index" aria-label="Agent 上报模块"><h3>对话主区域</h3><h3>结构化上报摘要</h3><h3>上报预收集</h3><p className="prototype-unavailable">摘要内容将在正式 Agent 运行完成后由真实运行结果填充。</p></section>
