...
export function BiDashboardPage() {
  ...
  return <Page title="驾驶舱 BI">
    ...
    <State state={state}>{(value) => <>
      <section className="bi-section" aria-labelledby="bi-summary-heading"><header className="section-heading"><div><h3 id="bi-summary-heading">管理摘要</h3><p>整体经营与运维表现，数据来自正式 BI 接口。</p></div></header><div className="data-grid bi-kpi-grid"><div className="metric bi-kpi"><div className="bi-kpi__head"><span className="label">设备健康综合评分</span><span className="bi-kpi__icon" aria-hidden="true">♥</span></div><b>—</b><span className="bi-kpi__delta">当前 API 未提供健康评分</span><div className="bi-kpi__sparkline" aria-hidden="true" /></div><div className="metric bi-kpi"><div className="bi-kpi__head"><span className="label">故障总数</span><span className="bi-kpi__icon" aria-hidden="true">!</span></div><b>{value.summary.fault_count}</b><span className="bi-kpi__delta">当前周期正式统计</span><div className="bi-kpi__sparkline" aria-hidden="true">{value.trend.slice(-5).map((item) => <i key={`fault-${item.date}`} style={{ height: `${Math.max(12, item.fault_count * 10)}%` }} />)}</div></div><div className="metric bi-kpi"><div className="bi-kpi__head"><span className="label">工单总数</span><span className="bi-kpi__icon" aria-hidden="true">▤</span></div><b>—</b><span className="bi-kpi__delta">当前 API 未提供工单总数</span><div className="bi-kpi__sparkline" aria-hidden="true" /></div><div className="metric bi-kpi"><div className="bi-kpi__head"><span className="label">工单按时完成率</span><span className="bi-kpi__icon" aria-hidden="true">✓</span></div><b>—</b><span className="bi-kpi__delta">当前 API 未提供按时完成率</span><div className="bi-kpi__sparkline" aria-hidden="true" /></div><div className="metric metric--warning bi-kpi"><div className="bi-kpi__head"><span className="label">平均维修时长 MTTR</span><span className="bi-kpi__icon" aria-hidden="true">◷</span></div><b>{value.efficiency.average_completion_hours ?? "—"}<small>{value.efficiency.average_completion_hours === null ? "" : " 小时"}</small></b><span className="bi-kpi__delta">当前周期正式统计</span><div className="bi-kpi__sparkline" aria-hidden="true">{value.trend.slice(-5).map((item) => <i key={`hours-${item.date}`} style={{ height: `${Math.max(12, Math.min(100, item.completed_work_order_count * 10))}%` }} />)}</div></div></div></section>
...
type EquipmentDetailTab = "graph" | "bom" | "params" | "docs" | "records";

function EquipmentDetailTabPanel({ activeTab, history }: { activeTab: EquipmentDetailTab; history: LoadState<{ items: MaintenanceRecord[]; count: number; page: number; page_size: number; trend: { date: string; completed_count: number }[] }> }) {
  if (activeTab === "graph") return <section className="equipment-graph-panel" aria-label="知识图谱"><div className="equipment-graph-legend" aria-label="知识图谱图例"><span><i className="kg-dot kg-dot--blue" />部件系统</span><span><i className="kg-dot kg-dot--green" />额定参数</span><span><i className="kg-dot kg-dot--purple" />知识文档</span><span><i className="kg-dot kg-dot--orange" />维修记录</span></div><div className="equipment-graph-canvas" role="img" aria-label="设备知识图谱"><div className="equipment-graph-empty"><strong>设备知识图谱</strong><p>当前 API 未提供设备知识图谱数据。</p></div></div><aside className="equipment-graph-related" aria-label="关联内容"><h3>关联内容</h3><p className="prototype-unavailable">当前 API 未提供关联参数、BOM、文档和维修记录。</p></aside></section>;
  if (activeTab === "bom") return <section className="equipment-bom-panel" aria-label="BOM 组成"><div className="equipment-bom-tree" role="tree" aria-label="BOM 组成"><p className="prototype-unavailable">当前 API 未提供 BOM 数据。</p></div><article className="equipment-bom-detail"><h3>BOM 节点详情</h3><dl className="detail-list"><dt>节点层级</dt><dd>当前 API 未提供</dd><dt>节点编码</dt><dd>当前 API 未提供</dd><dt>节点名称</dt><dd>当前 API 未提供</dd><dt>单位与用量</dt><dd>当前 API 未提供</dd></dl></article></section>;
  if (activeTab === "params") return <section className="equipment-tab-table" aria-label="额定参数"><table aria-label="额定参数"><thead><tr><th>序号</th><th>参数名</th><th>额定参数值</th><th>浮动上限</th><th>浮动下限</th><th>单位</th><th>说明</th></tr></thead><tbody /></table><p className="empty-panel" role="status">当前 API 未提供额定参数数据。</p></section>;
  if (activeTab === "docs") return <section className="equipment-tab-table" aria-label="知识文档"><table aria-label="知识文档"><thead><tr><th>序号</th><th>文件类型</th><th>文件名</th><th>文件大小</th><th>上传时间</th><th>文件格式</th><th>向量化状态</th></tr></thead><tbody /></table><p className="empty-panel" role="status">当前 API 未提供知识文档数据。</p><p className="prototype-unavailable">知识文档接口尚未提供，当前仅展示正式字段状态。</p></section>;
  return <State state={history}>{(data) => <section aria-label="设备维修记录">{data.items.length ? <RecordsTable items={data.items} /> : <p className="empty-panel" role="status">暂无维修历史记录。</p>}<section className="history-trend" aria-label="维修完成趋势"><h4>维修完成趋势</h4>{data.trend.length ? <ul>{data.trend.map((item) => <li key={item.date}>{item.date}：完成 {item.completed_count} 次</li>)}</ul> : <p>暂无维修完成趋势。</p>}</section></section>}</State>;
}
...
export function EquipmentDetailPage() {
  ...
  const health = useData(() => getHealthScore(id), [id]);
  const [activeTab, setActiveTab] = useState<EquipmentDetailTab>("graph");
  const [healthOpen, setHealthOpen] = useState(false);
  return <Page title="设备详情"><State state={equipment}>{(item) => <>
    <article className="equipment-detail-overview-card"><div className="equipment-photo" aria-label="设备图片"><div className="equipment-photo-body"><span>EV</span><small>{(item.image_refs ?? [])[0]?.filename ?? "当前 API 未提供设备图片"}</small></div></div><div className="detail-overview-main"><div className="overview-title-row"><h3>{item.code} · {item.name}（{item.model}）</h3><span className={`status-chip ${item.status === "FAULT" ? "status-chip--danger" : item.status === "NORMAL" ? "status-chip--success" : "status-chip--neutral"}`}>{item.status}</span><span className="overview-tag info">知识已配置：当前 API 未提供状态</span><p className="muted small">所属组织：{item.organization_id}</p></div><div className="readonly-meta-grid"><div><span>负责人</span><strong>{item.owner_user_id ?? "未分配"}</strong></div><div><span>最近异常</span><strong className="prototype-unavailable">当前 API 未提供</strong></div><div><span>维修中工单</span><strong className="prototype-unavailable">当前 API 未提供</strong></div><div><span>待接单</span><strong className="prototype-unavailable">当前 API 未提供</strong></div><div><span>BOM 节点</span><strong className="prototype-unavailable">当前 API 未提供</strong></div><div><span>额定参数</span><strong className="prototype-unavailable">当前 API 未提供</strong></div><div><span>知识文档</span><strong className="prototype-unavailable">当前 API 未提供</strong></div><div><span>维修记录</span><strong className="prototype-unavailable">当前 API 未提供</strong></div></div></div><button className="health-ring-wrap" type="button" aria-label="查看健康评分明细" aria-expanded={healthOpen} onClick={() => setHealthOpen(true)}><div className="health-ring" style={{"--health-progress": `${health.value?.score ?? 0}%` } as React.CSSProperties}><div className="health-ring-score">{health.value?.score ?? "—"}</div></div><span className="health-ring-label">健康分</span></button><div className="detail-health-card"><div className="side-mini-card"><span>风险等级</span><strong>{health.value?.status ?? "当前 API 未提供"}</strong></div><div className="side-mini-card"><span>累计工时</span><strong>{item.operating_hours}</strong></div><div className="side-mini-card"><span>制造日期</span><strong>{item.manufactured_at ?? "未登记"}</strong></div><div className="side-mini-card"><span>投用日期</span><strong>{item.commissioned_at ?? "未登记"}</strong></div></div></article>
    {healthOpen && <section className="equipment-health-drawer" role="dialog" aria-modal="true" aria-labelledby="health-score-title"><header><div><strong id="health-score-title">健康评分明细</strong><div className="muted small">{item.code} · {item.name}</div></div><button className="icon-button" type="button" aria-label="关闭健康评分明细" onClick={() => setHealthOpen(false)}>×</button></header><div className="equipment-health-drawer__body">{health.value?.score === undefined ? <p className="prototype-unavailable">当前 API 未提供健康评分构成。</p> : <><strong className="health-score-value">{health.value.score} 分</strong><p>当前健康评分 · 风险状态：{health.value.status}</p></>}</div></section>}
  </>}</State><section className="equipment-detail-tabs" aria-label="设备详情分区"><div className="section-tabs" role="tablist" aria-label="设备详情标签页">{[["graph", "图谱关系"], ["bom", "BOM 组成"], ["params", "额定参数"], ["docs", "知识文档"], ["records", "维修记录"]].map(([idValue, label]) => <button key={idValue} type="button" role="tab" aria-selected={activeTab === idValue} onClick={() => setActiveTab(idValue as EquipmentDetailTab)}>{label}</button>)}</div><div className="equipment-detail-tab-panel" role="tabpanel"><EquipmentDetailTabPanel activeTab={activeTab} history={history} /></div></section><section className="history-panel" aria-label="维修历史"><header><div><h3>维修历史</h3><p>工单、故障和维修结论来自正式维修记录接口。</p></div></header><EquipmentDetailTabPanel activeTab="records" history={history} /></section></Page>;
}
function EquipmentForm({ edit = false }: { edit?: boolean }) {
  ...
  const saveLabel = edit ? "保存修改" : "保存设备";
  return <Page title={edit ? "编辑设备" : "新增设备"}><form className="equipment-form" onSubmit={submit}><header><div><p className="page-shell__eyebrow">{edit ? "维护已有资产字段" : "登记正式设备资产"}</p><h3>{edit ? current?.name ?? "编辑设备" : "新增设备"}</h3><p>保存只提交当前表单中的正式设备字段。</p></div><div className="equipment-form__header-actions"><Link className="button-secondary" to={edit ? `/equipment/${id}` : "/equipment"}>返回台账</Link><button type="submit" className="button-primary" disabled={saving}>{saving ? "保存中…" : saveLabel}</button></div></header><fieldset><legend>设备基础信息</legend><div className="portal-form"><label><span>设备编号 <em aria-hidden="true">*</em></span><input aria-label="设备编码" name="code" required defaultValue={current?.code} /></label><label><span>设备名称 <em aria-hidden="true">*</em></span><input aria-label="设备名称" name="name" required defaultValue={current?.name} /></label><label><span>设备型号 <em aria-hidden="true">*</em></span><input aria-label="型号" name="model" required defaultValue={current?.model} /></label><label>类型<input name="type" required defaultValue={current?.type} /></label><label>制造商<input name="manufacturer" required defaultValue={current?.manufacturer} /></label><label><span>所属工厂 <em aria-hidden="true">*</em></span><select aria-label="所属工厂" value={factoryId} onChange={(event) => { setFactoryId(event.target.value); setWorkshopId(""); setLineId(""); }}><option value="" disabled>请选择启用工厂</option>{factories.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label><span>所属车间 <em aria-hidden="true">*</em></span><select aria-label="所属车间" value={workshopId} onChange={(event) => { setWorkshopId(event.target.value); setLineId(""); }} disabled={factories.length > 0 && !factoryId}><option value="">请选择启用车间</option>{workshops.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label><span>所属产线 <em aria-hidden="true">*</em></span><select name="organization_id" aria-label="所属产线" value={effectiveLineId} onChange={(event) => setLineId(event.target.value)} required disabled={factories.length > 0 && !factoryId}><option value="" disabled>请选择启用产线</option>{(visibleLines.length ? visibleLines : lines).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label></div></fieldset><fieldset><legend>运行与归属</legend><div className="portal-form"><label><span>负责人（可多选）<em aria-hidden="true">*</em></span><select name="owner_user_id" aria-label="负责人" defaultValue={current?.owner_user_id ?? ""}><option value="">未分配</option>{owners.map((item) => <option key={item.id} value={item.id}>{item.username}</option>)}</select><small className="field-hint">当前正式 API 仅支持单一负责人，保存时按所选账号提交。</small></label><label>运行工时<input name="operating_hours" type="number" min="0" defaultValue={current?.operating_hours ?? 0} /></label><label>状态<select name="status" defaultValue={current?.status ?? "NORMAL"}><option>NORMAL</option><option>FAULT</option><option>REPAIRING</option><option>DISABLED</option></select></label><label>制造日期<input aria-label="制造日期" name="manufactured_at" type="date" defaultValue={current?.manufactured_at ?? ""} /></label><label>投用日期<input aria-label="投用日期" name="commissioned_at" type="date" defaultValue={current?.commissioned_at ?? ""} /></label></div></fieldset><fieldset><legend>设备图片引用</legend><div className="portal-form"><label>图片对象键<input aria-label="图片对象键" name="image_object_key" defaultValue={currentImage?.object_key ?? ""} /></label><label>图片文件名<input aria-label="图片文件名" name="image_filename" defaultValue={currentImage?.filename ?? ""} /></label></div></fieldset><section className="equipment-add-section"><div className="section-title"><h3>设备 BOM 组成</h3><button type="button" className="button-secondary" disabled title="当前 API 未提供 BOM 数据">新增分支节点</button></div><div className="prototype-unavailable"><p>当前 API 未提供该模块数据，未生成演示内容。</p><table aria-label="设备 BOM 组成字段"><thead><tr><th>层级</th><th>上级节点</th><th>分枝节点编码</th><th>分枝节点名称</th><th>描述</th><th>单位</th><th>BOM 用量</th><th>操作</th></tr></thead></table></div></section><section className="equipment-add-section"><div className="section-title"><h3>设备额定参数</h3><button type="button" className="button-secondary" disabled title="当前 API 未提供额定参数数据">新增参数</button></div><div className="prototype-unavailable"><p>当前 API 未提供该模块数据，未生成演示内容。</p><table aria-label="设备额定参数字段"><thead><tr><th>序号</th><th>参数名</th><th>额定参数值</th><th>浮动上限</th><th>浮动下限</th><th>单位</th><th>说明</th><th>操作</th></tr></thead></table></div></section><section className="equipment-add-section"><div className="section-title"><h3>知识资料</h3><button type="button" className="button-secondary" disabled title="当前 API 未提供知识资料上传接口">上传资料</button></div><div className="prototype-unavailable"><p>当前 API 未提供该模块数据，未生成演示内容。</p><table aria-label="知识资料字段"><thead><tr><th>序号</th><th>资料类型</th><th>文件名</th><th>大小</th><th>上传时间</th><th>格式</th><th>操作</th></tr></thead></table></div></section><section className="equipment-add-section equipment-upload-panel"><div className="section-title"><h3>上传知识资料</h3><span className="muted small">当前 API 未提供知识资料上传接口</span></div><div className="equipment-upload-panel__body"><label>文件类型<select aria-label="知识资料文件类型" disabled><option>维修手册</option><option>故障手册</option><option>历史案例</option><option>点检标准</option><option>安全规范</option><option>其他资料</option></select></label><label className="file-upload">选择 Word / PDF 文件<input aria-label="选择 Word / PDF 文件" type="file" accept=".doc,.docx,.pdf" disabled /><span>未选择文件</span></label></div><div className="equipment-modal-actions"><button type="button" className="button-secondary" disabled>取消</button><button type="button" className="button-primary" disabled title="当前 API 未提供知识资料上传接口">确认上传</button></div></section><footer><Link className="button-secondary" to={edit ? `/equipment/${id}` : "/equipment"}>取消</Link><button type="submit" className="button-primary" disabled={saving}>{saving ? "保存中…" : saveLabel}</button>{!effectiveLineId && <p role="alert">请选择可用产线，无法保存设备。</p>}{message && <p role="status">{message}</p>}</footer></form></Page>;
}
...
function MaintenancePrototypeTable({ items }: { items: MaintenanceRecord[] }) {
  return <table aria-label="维修记录列表"><thead><tr><th>序号</th><th>维修记录编号</th><th>所属车间</th><th>所属产线</th><th>异常信息</th><th>工单状态</th><th>维修负责人</th><th>处理结果</th><th>完成时间</th><th>操作</th></tr></thead><tbody>{items.map((item, index) => <tr key={item.maintenance_record_id}><td>{index + 1}</td><td>{item.maintenance_record_id}</td><td><span className="prototype-unavailable table-unavailable">当前 API 未提供</span></td><td><span className="prototype-unavailable table-unavailable">当前 API 未提供</span></td><td>{item.symptom || "未填写"}</td><td>{item.status}</td><td><span className="prototype-unavailable table-unavailable">当前 API 未提供</span></td><td>{item.repair_result ?? "未完成"}</td><td>{item.completed_at ?? "—"}</td><td><button type="button" className="link-button" aria-label={`查看 ${item.maintenance_record_id} 维修详情`} data-maintenance-record-id={item.maintenance_record_id}>详情</button></td></tr>)}</tbody></table>;
}
...
function SystemManagementContent({ canWrite }: { canWrite: boolean }) {
  ...
  const organizations = useData(getOrganizations, [refresh]);
  ...
  return <Page title="系统管理">
    ...
    {section === "users" && <><div className="system-users-layout"><aside className="data-card system-org-tree"><header className="panel-heading"><div><h3>组织树</h3><p>组织结构来自正式组织接口。</p></div></header><div className="system-org-tree__actions"><button type="button" className="button-primary" disabled title="当前 API 未提供当前上下文的组织创建入口">新增组织</button><button type="button" className="button-secondary" disabled title="当前 API 未提供当前上下文的组织删除入口">删除组织</button></div><State state={organizations} empty={(items) => !items.length}>{(items) => <ul>{items.map((item) => <li key={item.id}><span>{item.name} {item.code}</span><small>{item.enabled ? "启用" : "停用"}</small></li>)}</ul>}</State></aside><section className="system-users-main"><div className="panel-heading"><div><h3>用户列表</h3><p>仅身份写入权限可新增或调整账号状态。</p></div><span className={`status-chip ${canWrite ? "status-chip--success" : "status-chip--neutral"}`}>{canWrite ? "可管理" : "只读"}</span></div>
    ...
    </>}</State></section></div><section className="data-card system-self-account"><header className="panel-heading"><div><h3>我的账号</h3><p>当前账号信息来自认证会话；敏感凭据不会在页面回显。</p></div><span className="status-chip status-chip--neutral">仅本人</span></header><p className="prototype-unavailable">当前页面未提供当前用户详情接口。</p></section></>}
    {section === "roles" && <><div className="panel-heading"><div><h3>角色列表</h3><p>支持按角色名称查看与配置正式权限。</p></div><button type="button" className="button-primary" disabled title="当前 API 未提供角色创建接口">新增角色</button></div><State state={roles} empty={(items) => !items.length}>{(items) => <><table className="system-role-table"><thead><tr><th>角色名称</th><th>角色类型</th><th>状态</th><th>绑定用户</th><th>更新时间</th><th>操作</th></tr></thead><tbody>{items.map((item) => <tr key={item.id}><td>{item.name}</td><td>{item.code}</td><td><span className="prototype-unavailable">当前 API 未提供</span></td><td><span className="prototype-unavailable">当前 API 未提供</span></td><td><span className="prototype-unavailable">当前 API 未提供</span></td><td><button type="button" disabled={!canWrite} onClick={() => setSelectedRoleId(item.id)}>配置权限</button></td></tr>)}</tbody></table>{selectedRoleId && items.filter((item) => item.id === selectedRoleId).map((item) => <RolePermissionEditor key={item.id} role={item} permissions={permissions.value ?? []} canWrite={canWrite} onSaved={() => { setRefresh((value) => value + 1); setNotice("角色权限已更新。"); }} onFailed={() => setNotice("角色更新失败，请检查权限后重试。")} />)}</>}</State></>}
    {(section === "loginLogs" || section === "operationLogs") && <><div className="panel-heading"><div><h3>{section === "loginLogs" ? "登录日志" : "操作日志"}</h3><p>只展示服务端已保留的审计记录。</p></div></div><label>动作筛选<input aria-label="审计动作筛选" value={auditAction} onChange={(event) => { setAuditAction(event.target.value); setAuditPage(1); }} placeholder="输入正式动作" /></label><State state={audits} empty={(data) => !data.count}>{(data) => <><table className="system-audit-table"><thead>{section === "loginLogs" ? <tr><th>登录账号</th><th>姓名</th><th>登录时间</th><th>来源 IP</th><th>登录终端</th><th>结果</th><th>说明</th></tr> : <tr><th>操作时间</th><th>操作人</th><th>模块</th><th>操作类型</th><th>操作对象</th><th>结果</th><th>说明</th></tr>}</thead><tbody>{data.items.map((item: AuditEvent) => section === "loginLogs" ? <tr key={item.id}><td className="prototype-unavailable">当前 API 未提供</td><td className="prototype-unavailable">当前 API 未提供</td><td>{item.created_at}</td><td className="prototype-unavailable">当前 API 未提供</td><td className="prototype-unavailable">当前 API 未提供</td><td>{item.result}</td><td>{item.action}</td></tr> : <tr key={item.id}><td>{item.created_at}</td><td className="prototype-unavailable">当前 API 未提供</td><td className="prototype-unavailable">当前 API 未提供</td><td>{item.action}</td><td>{item.resource_type}</td><td>{item.result}</td><td className="prototype-unavailable">当前 API 未提供</td></tr>)}</tbody></table><div className="pager"><button type="button" disabled={auditPage <= 1} onClick={() => setAuditPage((current) => current - 1)}>上一页</button><span>第 {auditPage} 页</span><button type="button" disabled={data.items.length < data.page_size} onClick={() => setAuditPage((current) => current + 1)}>下一页</button></div></>}</State></>}
    </section>
  </Page>;
}

function RolePermissionEditor({ role, permissions, canWrite, onSaved, onFailed }: { role: { id: string; code: string; name: string; permission_codes: string[] }; permissions: Array<{ code: string }>; canWrite: boolean; onSaved: () => void; onFailed: () => void }) {
...
export function AgentReportPage() {
  ...
  return <Page title="AI 故障上报">
    <div className="agent-report-layout"><section className="data-card agent-chat-panel" aria-label="对话主区域"><header className="section-heading"><div><h3>对话主区域</h3><p>AI 先校验设备权限，再整理为结构化上报单。</p></div><span className={`status-chip ${collected ? "status-chip--success" : "status-chip--warning"}`}>{collected ? "已完成" : "待收集"}</span></header><div className="agent-chat-flow"><p className="agent-bubble">请描述故障现象，我会先校验设备权限，再整理为结构化上报单。</p>{symptom && <p className="agent-bubble agent-bubble--user">{symptom}</p>}{runtimeEvents.length ? <p className="agent-bubble">运行状态：{String(runtimeEvents[runtimeEvents.length - 1].data.status ?? runtimeEvents[runtimeEvents.length - 1].event)}</p> : <p className="prototype-unavailable">尚未开始 Agent 运行，暂无实时对话结果。</p>}</div><form className="agent-input-row" onSubmit={collect}><label className="agent-input-field">设备 ID<input aria-label="设备 ID" value={equipmentId} onChange={(event) => setEquipmentId(event.target.value)} required /></label><label className="agent-input-field agent-input-field--message">故障描述<textarea aria-label="故障描述" value={symptom} onChange={(event) => setSymptom(event.target.value)} required /></label><label className="agent-attachment-trigger">附件<input aria-label="AI 故障附件" type="file" hidden disabled={uploading || submitting} onChange={(event) => void addAttachment(event.target.files?.[0])} /></label><button type="submit" className="button-primary" disabled={collecting || submitting || uploading}>{collecting ? "AI 收集中…" : "开始 AI 收集"}</button></form><section className="agent-progress-card" aria-label="上报预收集"><header className="section-heading"><h3>上报预收集</h3><span className="status-chip status-chip--warning">{collected ? "已完成" : "待收集"}</span></header><p>设备、故障现象和运行状态将在正式 Agent 完成后汇总。</p></section></section><aside className="data-card agent-summary-panel" aria-label="结构化上报摘要"><header className="section-heading"><div><h3>结构化上报摘要</h3><p>正式提交前由人工补齐并确认。</p></div><span className={`status-chip ${occurredAt && durationMinutes > 0 ? "status-chip--success" : "status-chip--warning"}`}>{occurredAt && durationMinutes > 0 ? "可提交" : "未完成"}</span></header><dl className="detail-list"><dt>已选设备</dt><dd>{equipmentId || "未选择"}</dd><dt>故障摘要</dt><dd>{symptom || "未填写"}</dd><dt>必填完成度</dt><dd>{[equipmentId, symptom, occurredAt, durationMinutes > 0 ? String(durationMinutes) : ""].filter(Boolean).length} / 4</dd><dt>提交状态</dt><dd>{collected ? "等待人工确认" : "等待 AI 收集"}</dd></dl><div className="drawer-actions"><Link className="button-secondary" to="/fault-report">转人工表单</Link><button type="button" className="button-primary" disabled={!collected || submitting || uploading || !occurredAt || durationMinutes <= 0} onClick={() => setNotice("请在下方补齐正式字段后提交。")}>提交故障单</button></div>{collected && <section className="agent-confirmation-panel"><h3 aria-label="正式字段确认">正式字段确认</h3><form className="portal-form" onSubmit={confirm}><label>紧急程度<select value={urgency} onChange={(event) => setUrgency(event.target.value)}><option>HIGH</option><option>MEDIUM</option><option>LOW</option></select></label><label>发生时间<input aria-label="发生时间" type="datetime-local" value={occurredAt} onChange={(event) => setOccurredAt(event.target.value)} required /></label><label>持续时间（分钟）<input aria-label="持续时间（分钟）" type="number" min="0" value={durationMinutes} onChange={(event) => setDurationMinutes(Number(event.target.value))} /></label><label>补充说明<textarea value={description} onChange={(event) => setDescription(event.target.value)} /></label><div className="form-actions"><button type="button" className="button-secondary" onClick={() => { setCollected(false); setNotice("已返回上报预收集，可继续补充现场信息。"); }}>继续补充</button><button type="submit" disabled={submitting || uploading || !occurredAt || durationMinutes <= 0}>{submitting ? "提交中…" : "确认并提交正式故障单"}</button></div></form></section>}</aside></div>
