...
function SystemManagementContent({ canWrite }: { canWrite: boolean }) {
  ...
  return <Page title="系统管理">
    ...
    <section className="section-tab-panel system-tab-panel" role="tabpanel">
      {section === "roles" && <><div className="system-toolbar system-role-toolbar"><label className="system-role-search"><span aria-hidden="true">⌕</span><input aria-label="搜索角色名称" value={roleQuery} onChange={(event) => setRoleQuery(event.target.value)} placeholder="搜索角色名称" /></label><div className="system-role-filters"><span className="system-filter-chip system-filter-chip--active">内置角色仅系统管理员</span><span className="system-filter-chip">自定义角色可自由新增</span></div><div><button type="button" className="button-primary" disabled title="当前公开 API 未提供角色创建">新增角色</button><button type="button" className="button-secondary" onClick={() => setRefresh((value) => value + 1)}>刷新</button></div></div><div className="panel-heading system-role-heading"><div><h3>角色列表</h3><p>支持按角色名称搜索；已绑定用户的自定义角色不可删除。</p></div><span className="status-chip status-chip--success">● {roleItems.length} 个角色</span></div><State state={roles} empty={(items) => !items.length}>{() => <>{roleItems.length ? <table className="system-role-table"><thead><tr><th>角色名称</th><th>角色类型</th><th>状态</th><th>绑定用户</th><th>更新时间</th><th>操作</th></tr></thead><tbody>{roleItems.map((role) => { const builtin = isBuiltinRole(role); const bound = roleBoundCount(role.id); return <tr key={role.id}><td><strong>{role.name}</strong><small>{builtin ? "内置最高权限角色，禁止删除。" : "自定义角色"}</small></td><td><span className={`system-role-type ${builtin ? "is-builtin" : ""}`}>{builtin ? "内置角色" : "自定义角色"}</span></td><td><span className="system-status-enabled">● 启用</span></td><td><span className="system-bound-count">{bound} 人</span></td><td>2026-07-01 09:10</td><td className="system-role-actions">{canWrite ? <button type="button" className="link-button" onClick={() => setEditingRoleId((current) => current === role.id ? null : role.id)}>{editingRoleId === role.id ? "收起权限" : "编辑权限"}</button> : <span>只读</span>}<button type="button" className="link-button" disabled={!canWrite || builtin} title={builtin ? "内置角色禁止删除" : bound ? "已绑定用户的角色不可删除" : "当前公开 API 未提供角色删除"}>删除</button></td></tr>; })}</tbody></table> : <p className="empty-panel">没有符合条件的正式角色。</p>}{editingRoleId && roleItems.find((item) => item.id === editingRoleId) && <RolePermissionEditor role={roleItems.find((item) => item.id === editingRoleId)!} permissions={permissions.value ?? []} canWrite={canWrite} onSaved={() => { setRefresh((value) => value + 1); setNotice("角色权限已更新。"); }} onFailed={() => setNotice("角色更新失败，请检查权限后重试。")} />}<section className="system-permission-catalogue"><h4>权限配置目录</h4><State state={permissions} empty={(items) => !items.length}>{(items) => <div className="permission-catalogue">{items.map((item) => <code key={item.code}>{item.code}</code>)}</div>}</State></section></>}</State></>}
