...
function SystemManagementContent({ canWrite }: { canWrite: boolean }) {
  ...
  async function updateSelectedUser(roleIds: string[], enabled: boolean) {
    if (!selectedUser || saving || !canWrite) return;
    setSaving(true);
    try {
      await updateUser(selectedUser.id, { role_ids: roleIds, enabled });
      setUserModal(null);
      setRefresh((value) => value + 1);
      setNotice("账号信息已更新。");
    } catch {
  ...
  const roleItems = (roles.value ?? []).filter((item) => !roleQuery || [roleName(item), item.name, item.code].some((value) => value.toLowerCase().includes(roleQuery.toLowerCase())));
  const roleBoundCount = (roleId: string) => (users.value ?? []).filter((user) => user.role_ids.includes(roleId)).length;
  const isBuiltinRole = (role: { code: string }) => role.code.toLowerCase() === "admin" || role.code.toLowerCase().includes("system_admin");
  const userItems = (users.value ?? []).filter((item) => (!userQuery || item.username.toLowerCase().includes(userQuery.toLowerCase())) && (!userRoleId || item.role_ids.includes(userRoleId)));
  ...
  return <Page title="系统管理">
    ...
    <section className="section-tab-panel system-tab-panel" role="tabpanel">
      {section === "roles" && <RolePanel roles={roles} items={roleItems} canWrite={canWrite} page={rolePage} onPageChange={setRolePage} onSearch={setRoleQuery} onCreate={() => setRoleModal("new")} onRefresh={() => setRefresh((value) => value + 1)} onEdit={(id) => { setEditingRoleId(id); setRoleModal("edit"); }} roleName={roleName} isBuiltin={isBuiltinRole} roleBoundCount={roleBoundCount} />}
      ...
      {section === "loginLogs" && <PrototypeAuditLogPanel kind="login" data={audits} page={loginPage} query={loginQuery} result={loginResult} range={loginRange} onQuery={(value) => { setLoginQuery(value); setLoginPage(1); }} onResult={(value) => { setLoginResult(value); setLoginPage(1); }} onRange={(value) => { setLoginRange(value); setLoginPage(1); }} onPageChange={setLoginPage} onRefresh={() => { setLoginQuery(""); setLoginResult(""); setLoginRange(""); setLoginPage(1); setRefresh((value) => value + 1); }} />}
    ...
    {roleModal && <RoleEditorModal mode={roleModal} role={roleModal === "edit" ? roleItems.find((item) => item.id === editingRoleId) ?? roleItems[0] : undefined} permissions={permissions.value ?? []} canWrite={canWrite} onClose={() => setRoleModal(null)} onSaved={() => { setRoleModal(null); setRefresh((value) => value + 1); setNotice("角色权限已更新。"); }} onFailed={() => setNotice("角色权限更新失败，请检查权限后重试。")} />}
    {userModal && <UserModal mode={userModal} user={selectedUser} roles={roles.value ?? []} canWrite={canWrite} saving={saving} roleName={roleName} onClose={() => setUserModal(null)} onCreate={create} onUpdate={(roleIds, enabled) => void updateSelectedUser(roleIds, enabled)} />}
  </Page>;
}
...
function RolePanel({ roles, items, canWrite, page, onPageChange, onSearch, onCreate, onRefresh, onEdit, roleName, isBuiltin, roleBoundCount }: { roles: LoadState<Array<{ id: string; code: string; name: string; permission_codes: string[] }>>; items: Array<{ id: string; code: string; name: string; permission_codes: string[] }>; canWrite: boolean; page: number; onPageChange: React.Dispatch<React.SetStateAction<number>>; onSearch: (value: string) => void; onCreate: () => void; onRefresh: () => void; onEdit: (id: string) => void; roleName: (role: { code: string; name: string }) => string; isBuiltin: (role: { code: string }) => boolean; roleBoundCount: (id: string) => number }) {
  const rows = items.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE);
  return <><div className="system-toolbar"><div className="system-toolbar-left"><label className="system-search">⌕<input aria-label="搜索角色名称" placeholder="搜索角色名称" onChange={(event) => onSearch(event.target.value)} /></label><span className="system-tag blue">内置角色仅系统管理员</span><span className="system-tag">自定义角色可自由新增</span></div><div className="system-toolbar-actions"><button type="button" className="button-primary" disabled title={canWrite ? "当前公开 API 未提供新增角色" : "当前账号没有身份写入权限"} onClick={onCreate}>新增角色</button><button type="button" className="button-secondary" onClick={onRefresh}>刷新</button></div></div><article className="system-table-card"><div className="system-table-head"><div><h3>角色列表</h3><p>支持按角色名称搜索；已绑定用户的自定义角色不可删除。</p></div><span className="status-chip status-chip--success">{items.length} 个角色</span></div><State state={roles} empty={(value) => !value.length}>{() => <><div className="system-table-wrap"><table className="system-table"><thead><tr><th>角色名称</th><th>角色类型</th><th>状态</th><th>绑定用户</th><th>更新时间</th><th>操作</th></tr></thead><tbody>{rows.map((role) => { const builtin = isBuiltin(role); const bound = roleBoundCount(role.id); return <tr key={role.id}><td><strong>{roleName(role)}</strong><small>{builtin ? "内置最高权限角色，禁止删除。" : "自定义角色"}</small></td><td><span className={`system-role-type ${builtin ? "is-builtin" : ""}`}>{builtin ? "内置角色" : "自定义角色"}</span></td><td><span className="system-status-enabled">启用</span></td><td><span className="system-bound-count">{bound} 人</span></td><td>当前接口未提供</td><td><div className="system-link-actions">{canWrite ? <button type="button" className="system-text-action" onClick={() => onEdit(role.id)}>编辑权限</button> : <span>只读</span>}<button type="button" className="system-text-action danger" disabled title={builtin ? "内置角色禁止删除" : bound ? "已绑定用户的角色不可删除" : "当前公开 API 未提供角色删除"}>删除</button></div></td></tr>; })}</tbody></table></div><PrototypePager count={items.length} page={page} onPageChange={onPageChange} /></>}</State></article></>;
}

function UserPanel({ users, roles, items, canWrite, saving, page, query, roleId, onSearch, onRoleFilter, onPageChange, onRefresh, onCreate, onDetail, onEdit, onToggle, roleName }: { users: LoadState<Array<{ id: string; username: string; enabled: boolean; role_ids: string[] }>>; roles: Array<{ id: string; code: string; name: string }>; items: Array<{ id: string; username: string; enabled: boolean; role_ids: string[] }>; canWrite: boolean; saving: boolean; page: number; query: string; roleId: string; onSearch: (value: string) => void; onRoleFilter: (value: string) => void; onPageChange: React.Dispatch<React.SetStateAction<number>>; onRefresh: () => void; onCreate: () => void; onDetail: (id: string) => void; onEdit: (id: string) => void; onToggle: (item: { id: string; username: string; enabled: boolean; role_ids: string[] }) => void; roleName: (role: { code: string; name: string }) => string }) {
  const rows = items.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE);
...
function UserModal({ mode, user, roles, canWrite, saving, roleName, onClose, onCreate, onUpdate }: { mode: "new" | "edit" | "detail"; user?: { id: string; username: string; enabled: boolean; role_ids: string[] }; roles: Array<{ id: string; code: string; name: string }>; canWrite: boolean; saving: boolean; roleName: (role: { code: string; name: string }) => string; onClose: () => void; onCreate: (event: FormEvent<HTMLFormElement>) => Promise<void>; onUpdate: (roleIds: string[], enabled: boolean) => void }) {
  const [roleId, setRoleId] = useState(user?.role_ids[0] ?? roles[0]?.id ?? "");
  const [enabled, setEnabled] = useState(user?.enabled ?? true);
  const detail = mode === "detail";
  const submit = (event: FormEvent<HTMLFormElement>) => { if (mode === "new") void onCreate(event); else { event.preventDefault(); onUpdate(roleId ? [roleId] : [], enabled); } };
  return <div className="system-modal-layer" role="presentation"><div className="system-modal-scrim" onClick={onClose} /><section className="system-user-modal" role="dialog" aria-modal="true" aria-label={mode === "new" ? "新增用户" : mode === "edit" ? "编辑用户" : "用户详情"}><header className="system-role-modal__head"><div><h3>{mode === "new" ? "新增用户" : mode === "edit" ? "编辑用户" : "用户详情"}</h3><p>{detail ? "展示列表隐藏字段与账号完整授权信息。" : "用户名全局唯一；编辑用户时用户名保持只读。"}</p></div><button type="button" className="icon-button" aria-label="关闭用户弹窗" onClick={onClose}>×</button></header><form className="system-user-form" onSubmit={submit}><div className="form-grid-3"><label>用户名 <span className="required-mark">*</span><input aria-label="用户名" name="username" defaultValue={user?.username ?? ""} readOnly={mode === "edit" || detail} required disabled={detail} placeholder="例如：zhangsan" /></label><label>初始密码 <span className="required-mark">*</span><input aria-label="初始密码" name="password" type="password" minLength={8} required={mode === "new"} disabled={mode !== "new" || detail} placeholder="至少 8 位" /></label><label>姓名<input aria-label="姓名" disabled value="当前接口未提供" readOnly /></label><label>性别<select aria-label="性别" disabled><option>当前接口未提供</option></select></label><label>邮箱<input aria-label="邮箱" disabled value="当前接口未提供" readOnly /></label><label>手机号<input aria-label="手机号" disabled value="当前接口未提供" readOnly /></label><label>账号状态 <span className="required-mark">*</span><select aria-label="账号状态" value={enabled ? "enabled" : "disabled"} disabled={detail || !canWrite} onChange={(event) => setEnabled(event.target.value === "enabled")}><option value="enabled">启用</option><option value="disabled">禁用</option></select></label><label className="form-field--wide">授权角色 <span className="required-mark">*</span><select aria-label="授权角色" name="role_id" value={roleId} disabled={detail || !canWrite} onChange={(event) => setRoleId(event.target.value)}>{roles.map((role) => <option key={role.id} value={role.id}>{roleName(role)}</option>)}</select></label><label className="form-field--wide">备注<textarea disabled value="当前接口未提供" readOnly /></label></div><p className="system-boundary-note">当前用户 API 仅支持用户名、初始密码、授权角色和账号启停；其余原型字段保留为不可编辑状态。</p><footer className="system-role-modal__foot"><button type="button" className="button-secondary" onClick={onClose}>{detail ? "关闭" : "取消"}</button>{!detail && <button type="submit" className="button-primary" disabled={!canWrite || saving}>{saving ? "保存中…" : "保存用户"}</button>}</footer></form></section></div>;
}
...
function PrototypeAuditLogPanel({ kind, data, page, query, result, range, module, onQuery, onResult, onRange, onModule, onPageChange, onRefresh }: { kind: "login" | "operation"; data: LoadState<{ items: AuditEvent[]; count: number; page: number; page_size: number }>; page: number; query: string; result: string; range?: string; module?: string; onQuery: (value: string) => void; onResult: (value: string) => void; onRange?: (value: string) => void; onModule?: (value: string) => void; onPageChange: React.Dispatch<React.SetStateAction<number>>; onRefresh: () => void }) {
  ...
  const items = source.filter((item) => { const loginMatch = !login || item.action.toLowerCase().includes("login"); const searchable = [item.actor_user_id, item.action, item.resource_type, item.resource_id, item.result].filter(Boolean).join(" ").toLowerCase().includes(query.toLowerCase()); const resultMatch = !result || item.result.toLowerCase() === result; const moduleMatch = login || !module || item.resource_type === module; return loginMatch && searchable && resultMatch && moduleMatch; });
  const rows = items.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE);
  return <><div className="system-toolbar"><div className="system-toolbar-left"><label className="system-search">⌕<input aria-label={`${title}搜索`} value={query} onChange={(event) => onQuery(event.target.value)} placeholder={login ? "搜索用户名、姓名、IP" : "搜索操作人、模块、对象"} /></label>{login ? <><select className="system-filter-select log-filter-select" aria-label="按登录结果筛选" value={result} onChange={(event) => onResult(event.target.value)}><option value="">全部结果</option><option value="success">成功</option><option value="failure">失败</option></select><select className="system-filter-select log-filter-select" aria-label="按时间范围筛选" value={range} disabled title="当前审计 API 未提供服务端时间范围查询" onChange={(event) => onRange?.(event.target.value)}><option value="">全部时间</option><option value="today">今天</option><option value="week">近 7 天</option></select></> : <><select className="system-filter-select log-filter-select" aria-label="按模块筛选" value={module} onChange={(event) => onModule?.(event.target.value)}><option value="">全部模块</option>{[...new Set(source.map((item) => item.resource_type))].map((value) => <option key={value} value={value}>{value}</option>)}</select><select className="system-filter-select log-filter-select" aria-label="按操作结果筛选" value={result} onChange={(event) => onResult(event.target.value)}><option value="">全部结果</option><option value="success">成功</option><option value="failure">失败</option></select></>}</div><div className="system-toolbar-actions"><button type="button" className="button-secondary" onClick={onRefresh}>刷新</button></div></div><article className="system-table-card"><div className="system-table-head"><div><h3>{title}</h3><p>{login ? "仅用于审计查询，记录账号登录时间、来源 IP、终端和登录结果。" : "仅用于审计查询，记录角色、用户、组织等配置变更动作。"}</p></div><span className="status-chip status-chip--success">{items.length} 条记录</span></div><State state={data}>{() => <><p className="system-boundary-note">{login ? "当前接口未提供姓名、来源 IP、登录终端和说明。" : "当前接口未提供操作人名称和审计说明。"}</p><div className="system-table-wrap"><table className="system-table log-table"><thead>{login ? <tr><th>登录账号</th><th>姓名</th><th>登录时间</th><th>来源 IP</th><th>登录终端</th><th>结果</th><th>说明</th></tr> : <tr><th>操作时间</th><th>操作人</th><th>模块</th><th>操作类型</th><th>操作对象</th><th>结果</th><th>说明</th></tr>}</thead><tbody>{rows.length ? rows.map((item) => login ? <tr key={item.id}><td>{item.actor_user_id ?? "系统"}</td><td>当前接口未提供</td><td>{item.created_at}</td><td>当前接口未提供</td><td>当前接口未提供</td><td><span className={item.result === "success" ? "system-status-enabled" : "system-tag red"}>{item.result === "success" ? "成功" : "失败"}</span></td><td>当前接口未提供</td></tr> : <tr key={item.id}><td>{item.created_at}</td><td>{item.actor_user_id ?? "系统"}</td><td><span className="system-tag blue">{item.resource_type}</span></td><td>{item.action}</td><td>{item.resource_id ?? "当前接口未提供"}</td><td><span className={item.result === "success" ? "system-status-enabled" : "system-tag red"}>{item.result === "success" ? "成功" : "失败"}</span></td><td>当前接口未提供</td></tr>) : <tr><td colSpan={7}>暂无符合条件的正式{title}。</td></tr>}</tbody></table></div><PrototypePager count={items.length} page={page} onPageChange={onPageChange} /></>}</State></article></>;
}

function RoleEditorModal({ mode, role, permissions, canWrite, onClose, onSaved, onFailed }: { mode: "new" | "edit"; role?: { id: string; name: string; permission_codes: string[] }; permissions: Array<{ code: string }>; canWrite: boolean; onClose: () => void; onSaved: () => void; onFailed: () => void }) {
