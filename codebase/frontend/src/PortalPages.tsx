...
export function FactoryModelingPage({ permissionCodes }: { permissionCodes?: string[] } = {}) {
  ...
  return <Page title="工厂建模"><p>组织结构由正式组织 API 提供。</p>{notice && <p role="status">{notice}</p>}{!canWrite && <p role="status">当前账号只有组织读取权限，不能修改组织结构。</p>}<label>搜索组织<input aria-label="搜索组织" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="按编码或名称筛选" /></label><State state={state} empty={(items) => !items.length}>{(items) => <><OrganizationTree items={items as OrganizationItem[]} query={query} collapsed={collapsed} canWrite={canWrite} onToggle={(id) => setCollapsed((current) => current.includes(id) ? current.filter((value) => value !== id) : [...current, id])} onEdit={setEditing} onStateChange={toggle} onDelete={remove} /><h3>新增组织节点</h3><form className="portal-form" onSubmit={submit}><fieldset disabled={!canWrite || saving}><label>类型<select name="type"><option>FACTORY</option><option>WORKSHOP</option><option>LINE</option></select></label><label>编码<input name="code" required /></label><label>名称<input name="name" required /></label><label>父节点 ID<input name="parent_id" required /></label><label>排序<input name="sort_order" type="number" min="0" defaultValue="0" /></label><label>备注<input name="remark" /></label><button type="submit">创建节点</button></fieldset></form>{editing && <form className="portal-form" onSubmit={saveEdit}><fieldset disabled={!canWrite || saving}><h3>编辑组织：{editing.name}</h3><label>编码<input name="code" required defaultValue={editing.code} /></label><label>名称<input name="name" required defaultValue={editing.name} /></label><label>排序<input name="sort_order" type="number" min="0" defaultValue={editing.sort_order ?? 0} /></label><label>备注<input name="remark" defaultValue={editing.remark ?? ""} /></label><button type="submit">保存组织</button></fieldset><button type="button" onClick={() => setEditing(null)}>取消</button></form>}</>}</State></Page>;
}

function OrganizationTree({ items, query, collapsed, canWrite, onToggle, onEdit, onStateChange, onDelete }: { items: OrganizationItem[]; query: string; collapsed: string[]; canWrite: boolean; onToggle: (id: string) => void; onEdit: (item: OrganizationItem) => void; onStateChange: (item: OrganizationItem) => void; onDelete: (item: OrganizationItem) => void }) {
...
export function AgentReportPage() {
  ...
  return <Page title="AI 故障上报">
    <p>AI 只负责受控收集；它不会直接写入故障事实。正式上报必须由用户完成结构化确认。</p>
    <form className="portal-form" onSubmit={collect}>
    ...
    {runtimeEvents.length > 0 && <section aria-label="AI 运行状态">{runtimeEvents.map((item, index) => <p key={`${item.event}-${index}`}>运行状态：{String(item.data.status ?? item.event)}</p>)}</section>}
