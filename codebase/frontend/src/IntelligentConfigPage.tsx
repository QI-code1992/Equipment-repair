...
function PrototypeUnavailableModules({ groupTitle }: { groupTitle: string }) {
  const group = prototypeConfigGroups.find((item) => item.title === groupTitle);
  if (!group) return null;
  return <section className="config-prototype-group" aria-label={`${groupTitle}原型模块`}><p className="config-prototype-group-title">{groupTitle}</p><div className="config-prototype-catalogue__grid">{group.modules.map((module) => <article key={module}><p className="config-prototype-module-name">{module}</p><p className="prototype-unavailable">当前 API 未提供该模块数据。</p></article>)}</div></section>;
}
...
export function IntelligentConfigPage({ permissionCodes = [] }: { permissionCodes?: string[] }) {
  ...
  return <section className="config-page" aria-labelledby="page-heading">
    ...
    <section id="config-panel-models" className="config-panel" role="tabpanel" hidden={activeTab !== "models"} aria-label="模型配置"><PrototypeUnavailableModules groupTitle="模型配置" /><section className="config-catalogue" aria-labelledby="catalogue-heading"><header><h3 id="catalogue-heading">模型与绑定</h3><p>提供商、模型绑定和 Agent 配置保持独立的正式管理边界。</p></header><div className="config-catalogue__grid"><section className="data-card" aria-labelledby="provider-heading"><h3 id="provider-heading">模型提供商</h3>
    ...
    <section id="config-panel-agents" className="config-panel" role="tabpanel" hidden={activeTab !== "agents"} aria-label="智能体配置"><PrototypeUnavailableModules groupTitle="智能体配置" /><section className="config-control-plane" aria-labelledby="agent-control-heading"><header><h3 id="agent-control-heading">Agent 控制面</h3><p>每个 Agent 仅使用自己的正式模型绑定与运行策略。</p></header><div className="config-layout"><div className="config-agent-list" aria-label="Agent 配置列表">{configs.map((config) => <button className={config.agent_id === selectedId ? "config-agent config-agent--active" : "config-agent"} key={config.agent_id} type="button" onClick={() => select(config.agent_id)}>{agentLabel(config.agent_id)}</button>)}</div>
    ...
    <section id="config-panel-knowledge" className="config-panel" role="tabpanel" hidden={activeTab !== "knowledge"} aria-label="知识库配置"><PrototypeUnavailableModules groupTitle="知识库配置" /><section className="data-card" aria-labelledby="knowledge-heading"><h3 id="knowledge-heading">知识文档入口</h3><p>仅提交安全扫描后的正式文档引用；页面不显示对象键、扫描内部细节或正文。</p>{!canWriteKnowledge && <p role="status">当前账号没有知识库写入权限。</p>}<label>正式 Dataset ID<input aria-label="正式 Dataset ID" disabled={!canWriteKnowledge} value={knowledgeDatasetId} onChange={(event) => setKnowledgeDatasetId(event.target.value)} /></label><label>上传知识文档<input aria-label="上传知识文档" type="file" disabled={!canWriteKnowledge || knowledgeUploading} onChange={(event) => void uploadKnowledge(event.target.files?.[0])} /></label>{knowledgeUploading && <p role="status">正在上传并扫描知识文档…</p>}</section></section>
    <section id="config-panel-calls" className="config-panel" role="tabpanel" hidden={activeTab !== "calls"} aria-label="调用记录"><PrototypeUnavailableModules groupTitle="调用记录" /></section>
    <section id="config-panel-tokens" className="config-panel" role="tabpanel" hidden={activeTab !== "tokens"} aria-label="Token 消耗统计"><PrototypeUnavailableModules groupTitle="Token 消耗统计" /></section>
  </section>;
}
