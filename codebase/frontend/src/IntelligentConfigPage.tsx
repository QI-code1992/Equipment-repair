...
export function IntelligentConfigPage({ permissionCodes = [] }: { permissionCodes?: string[] }) {
  ...
  return <section className="config-page" aria-labelledby="page-heading">
    ...
    {activePanel === "calls" && <section className="config-reference-panel" role="tabpanel" aria-label="调用记录"><h3>调用记录</h3><p>调用状态、保留期和配置 Token 预算由正式审计接口提供。</p><a className="button-primary" href="/intelligent-audit">查看正式调用审计</a></section>}
    {activePanel === "tokens" && <section className="config-reference-panel" role="tabpanel" aria-label="Token 消耗统计"><h3>Token 消耗统计</h3><p>当前正式接口仅提供受控调用记录及配置 Token 预算，未提供可用于图表统计的实际 Token 消耗明细。</p><a className="button-primary" href="/intelligent-audit">查看正式调用审计</a></section>}
  </section>;
}
