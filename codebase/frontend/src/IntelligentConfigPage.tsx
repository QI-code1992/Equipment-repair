...
export function IntelligentConfigPage({ permissionCodes = [] }: { permissionCodes?: string[] }) {
  ...
  return <section className="config-page">
    ...
    {activePanel === "models" && <section className="config-model-workspace" role="tabpanel" aria-label="模型配置" aria-labelledby="catalogue-heading">
      ...
      {modelEditorOpen && <aside className="model-editor" aria-label="模型编辑抽屉"><header><div><h3>{editingProvider ? "编辑模型提供商" : editingBinding ? `编辑模型：${editingBinding.name}` : "新增模型"}</h3><p>密钥仅以安全引用提交，不会在页面回显。</p></div><button type="button" aria-label="关闭模型编辑" onClick={() => { setModelEditorOpen(false); setEditingProvider(null); setEditingBinding(null); }}>关闭</button></header><div className="model-editor__body"><section><h4>模型提供商</h4>{editingProvider ? <form className="portal-form" onSubmit={saveProviderEdit}><label>提供商名称<input aria-label="编辑提供商名称" name="provider_name" defaultValue={editingProvider.name} required /></label><label>新的密钥引用<input aria-label="新的密钥引用" name="secret_ref" required autoComplete="off" /></label><label><input aria-label="启用编辑提供商" name="enabled" type="checkbox" defaultChecked={editingProvider.enabled} /> 启用提供商</label><button type="submit">保存模型提供商</button></form> : <form className="portal-form" onSubmit={addProvider}><label>提供商名称<input aria-label="提供商名称" name="provider_name" required /></label><label>密钥引用<input aria-label="密钥引用" name="secret_ref" required autoComplete="off" /></label><button type="submit">新增模型提供商</button></form>}{providers.length > 0 && <div className="provider-actions">{providers.map((provider) => <button type="button" key={provider.id} aria-label={`编辑模型提供商：${provider.name}`} onClick={() => { setEditingProvider(provider); setEditingBinding(null); }}>{provider.name}</button>)}</div>}</section><section><h4>模型绑定</h4>{editingBinding ? <form className="portal-form" onSubmit={saveBindingEdit}><label>提供商<select aria-label="编辑绑定提供商" name="provider_id" defaultValue={editingBinding.provider_id} required>{providers.filter((provider) => provider.enabled).map((provider) => <option key={provider.id} value={provider.id}>{provider.name}</option>)}</select></label><label>绑定名称<input aria-label="编辑绑定名称" name="binding_name" defaultValue={editingBinding.name} required /></label><label>模型名称<input aria-label="编辑模型名称" name="model_name" defaultValue={editingBinding.model_name} required /></label><label><input aria-label="编辑绑定支持深度思考" name="supports_reasoning" type="checkbox" defaultChecked={editingBinding.supports_reasoning} /> 支持深度思考</label><label><input name="enabled" type="checkbox" defaultChecked={editingBinding.enabled} /> 启用模型绑定</label><button type="submit">保存模型绑定</button></form> : <form className="portal-form" onSubmit={addBinding}><label>提供商<select name="provider_id" aria-label="绑定提供商" required defaultValue=""><option value="" disabled>请选择启用提供商</option>{providers.filter((provider) => provider.enabled).map((provider) => <option key={provider.id} value={provider.id}>{provider.name}</option>)}</select></label><label>绑定名称<input aria-label="绑定名称" name="binding_name" required /></label><label>模型名称<input aria-label="模型名称" name="model_name" required /></label><label><input name="supports_reasoning" type="checkbox" /> 支持深度思考</label><button type="submit" disabled={!providers.some((provider) => provider.enabled)}>新增模型绑定</button></form>}</section></div></aside>}
    </section>}
