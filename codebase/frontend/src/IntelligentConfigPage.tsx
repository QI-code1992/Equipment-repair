...
export function IntelligentConfigPage({ permissionCodes = [] }: { permissionCodes?: string[] }) {
  ...
  return <section className="config-page" aria-labelledby="page-heading">
    ...
    <section id="config-panel-knowledge" className="config-panel" role="tabpanel" hidden={activeTab !== "knowledge"} aria-label="知识库配置"><section className="data-card" aria-labelledby="knowledge-heading"><h3 id="knowledge-heading">知识文档入口</h3><p>仅提交安全扫描后的正式文档引用；页面不显示对象键、扫描内部细节或正文。</p>{!canWriteKnowledge && <p role="status">当前账号没有知识库写入权限。</p>}<label>正式 Dataset ID<input aria-label="正式 Dataset ID" disabled={!canWriteKnowledge} value={knowledgeDatasetId} onChange={(event) => setKnowledgeDatasetId(event.target.value)} /></label><label>上传知识文档<input aria-label="上传知识文档" type="file" disabled={!canWriteKnowledge || knowledgeUploading} onChange={(event) => void uploadKnowledge(event.target.files?.[0])} /></label>{knowledgeUploading && <p role="status">正在上传并扫描知识文档…</p>}</section></section>
    <section id="config-panel-calls" className="config-panel" role="tabpanel" hidden={activeTab !== "calls"} aria-label="调用记录"><section className="data-card"><h3>Agent 调用记录</h3><p className="prototype-unavailable">当前 API 未提供调用记录查询接口。</p></section></section>
    <section id="config-panel-tokens" className="config-panel" role="tabpanel" hidden={activeTab !== "tokens"} aria-label="Token 消耗统计"><section className="data-card"><h3>Token 消耗统计</h3><p className="prototype-unavailable">当前 API 未提供真实 Token 消耗统计接口。</p></section></section>
  </section>;
}
