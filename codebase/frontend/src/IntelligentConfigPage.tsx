...
export function IntelligentConfigPage({ permissionCodes = [] }: { permissionCodes?: string[] }) {
  ...
  return <section className="config-page" aria-labelledby="page-heading">
    ...
    <section id="config-panel-agents" className="config-panel" role="tabpanel" hidden={activeTab !== "agents"} aria-label="智能体配置"><section className="config-control-plane" aria-labelledby="agent-control-heading"><header><h3 id="agent-control-heading">智能体配置</h3><p>每个 Agent 仅使用自己的正式模型绑定与运行策略。</p></header><div className="config-layout"><div className="config-agent-list" aria-label="Agent 配置列表">{configs.map((config) => <button className={config.agent_id === selectedId ? "config-agent config-agent--active" : "config-agent"} key={config.agent_id} type="button" onClick={() => select(config.agent_id)}>{agentLabel(config.agent_id)}</button>)}</div>
