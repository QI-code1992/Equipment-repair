...
export function IntelligentConfigPage({ permissionCodes = [] }: { permissionCodes?: string[] }) {
  ...
  return <section className="config-page" aria-labelledby="page-heading">
    ...
    <section id="config-panel-agents" className="config-panel" role="tabpanel" hidden={activeTab !== "agents"} aria-label="智能体配置"><section className="config-control-plane" aria-labelledby="agent-control-heading"><header><h3 id="agent-control-heading">智能体配置</h3><p>每个 Agent 仅使用自己的正式模型绑定与运行策略。</p></header><div className="config-agent-grid" aria-label="Agent 配置卡片">{configs.map((config) => { const binding = bindings.find((item) => item.id === config.model_binding_id); return <article className="agent-card" key={config.agent_id}><header><h3>{agentLabel(config.agent_id)}</h3><span className={`status-chip ${config.enabled ? "status-chip--success" : "status-chip--warning"}`}>{config.enabled ? "已启用" : "停用"}</span></header><dl><div><dt>绑定 LLM</dt><dd>{binding ? binding.name : "未绑定"}</dd></div><div><dt>知识库数量</dt><dd>{config.knowledge_dataset_ids.length}</dd></div><div><dt>最近调用</dt><dd>当前 API 未提供</dd></div></dl><div className="agent-card__actions"><button type="button" className="button-primary" aria-label={`配置 Agent：${agentLabel(config.agent_id)}`} onClick={() => select(config.agent_id)}>配置</button><button type="button" aria-label={`测试 Agent：${agentLabel(config.agent_id)}`} disabled title="当前 API 未提供 Agent 配置测试接口">测试</button></div></article>; })}</div><div className="config-layout"><div className="config-agent-list" aria-label="Agent 配置列表">{configs.map((config) => <button className={config.agent_id === selectedId ? "config-agent config-agent--active" : "config-agent"} key={config.agent_id} type="button" onClick={() => select(config.agent_id)}>{agentLabel(config.agent_id)}</button>)}</div>
    ...
    <section id="config-panel-knowledge" className="config-panel" role="tabpanel" hidden={activeTab !== "knowledge"} aria-label="知识库配置"><div className="config-subtabs" role="tablist" aria-label="知识库配置二级页签"><button type="button" role="tab" aria-selected={knowledgeTab === "params"} onClick={() => setKnowledgeTab("params")}>知识库参数</button><button type="button" role="tab" aria-selected={knowledgeTab === "upload"} onClick={() => setKnowledgeTab("upload")}>知识上传</button></div>{knowledgeTab === "params" ? <><section className="data-card" aria-labelledby="knowledge-params-heading"><header className="section-heading"><div><h3 id="knowledge-params-heading">知识库参数</h3><p>解析策略、Embedding 与重排序配置。</p></div><button type="button" className="button-secondary" disabled title="当前 API 未提供知识库参数保存接口">配置参数</button></header><p className="prototype-unavailable">当前 API 未提供知识库参数、解析策略和混合检索配置接口。</p></section><section className="knowledge-catalogue" aria-labelledby="knowledge-catalogue-heading"><header className="section-heading"><div><h3 id="knowledge-catalogue-heading">知识库目录</h3><p>仅展示正式 Agent 绑定的知识库资源。</p></div><span className="status-chip status-chip--neutral">数据不可用</span></header><p className="prototype-unavailable">当前 API 未提供知识库目录数据。</p><button type="button" className="button-secondary" disabled title="当前 API 未提供知识库参数保存接口">配置参数</button></section><section className="data-card" aria-labelledby="knowledge-files-heading"><header className="section-heading"><div><h3 id="knowledge-files-heading">知识文件列表</h3><p>文件状态由正式 RAGFlow/Worker 生命周期提供。</p></div><button type="button" className="button-primary" disabled={!canWriteKnowledge} title={!canWriteKnowledge ? "当前账号没有知识库写入权限" : "请使用知识上传页提交文件"}>上传文件</button></header><table className="config-report-table"><thead><tr><th>文件名称</th><th>所属知识库</th><th>状态</th><th>解析进度</th><th>操作</th></tr></thead><tbody><tr><td colSpan={5} className="prototype-unavailable">当前 API 未提供知识文件列表数据。</td></tr></tbody></table></section></> : <section className="data-card" aria-labelledby="knowledge-heading"><h3 id="knowledge-heading">知识文档入口</h3><p>仅提交安全扫描后的正式文档引用；页面不显示对象键、扫描内部细节或正文。</p>{!canWriteKnowledge && <p role="status">当前账号没有知识库写入权限。</p>}<label>正式 Dataset ID<input aria-label="正式 Dataset ID" disabled={!canWriteKnowledge} value={knowledgeDatasetId} onChange={(event) => setKnowledgeDatasetId(event.target.value)} /></label><label>上传知识文档<input aria-label="上传知识文档" type="file" disabled={!canWriteKnowledge || knowledgeUploading} onChange={(event) => void uploadKnowledge(event.target.files?.[0])} /></label>{knowledgeUploading && <p role="status">正在上传并扫描知识文档…</p>}</section>}</section>
