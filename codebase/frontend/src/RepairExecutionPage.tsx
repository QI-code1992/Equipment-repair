...
export function RepairExecutionPage({ permissionCodes }: { permissionCodes?: string[] } = {}) {
  ...
  const [repairAttachmentNames, setRepairAttachmentNames] = useState<string[]>([]);
  ...
  return (
    <section className="page-shell" aria-labelledby="repair-execution-page-title">
      ...
      <aside className="data-card repair-summary-card" aria-label="工单摘要"><header className="section-heading"><div><h3>工单摘要</h3><p>身份、设备与当前状态来自正式工单 API。</p></div><span className="status-chip status-chip--warning">{selectedOrder?.status ?? "待选择"}</span></header><div className="repair-order-picker"><label>已分配工单<select aria-label="选择已分配工单" value={selectedOrder?.id ?? ""} onChange={(event) => { const order = assignedOrders?.find((item) => item.id === event.target.value); if (order) void selectOrder(order); }} disabled={assignedOrders === null || Boolean(ordersError)}><option value="">{assignedOrders === null ? "正在加载工单…" : assignedOrders.length === 0 ? "暂无已分配工单。" : "请选择工单"}</option>{assignedOrders?.map((order) => <option key={order.id} value={order.id}>{order.number} · {order.status}</option>)}</select></label><label>工单状态<select aria-label="维修执行工单状态筛选" value={orderStatus} onChange={(event) => { setOrderStatus(event.target.value); setAssignedOrders(null); setSelectedOrder(null); }}><option value="">全部</option><option value="PENDING_ACCEPT">待接单</option><option value="IN_REPAIR">维修中</option><option value="PENDING_INSPECTION">待验收</option><option value="COMPLETED">已完成</option></select></label>{ordersError && <p role="alert">{ordersError}</p>}</div><div className="repair-summary-list"><div><span className="repair-summary-icon">单</span><span>工单</span><strong>{selectedOrder?.number ?? "未选择"}</strong></div><div><span className="repair-summary-icon">设</span><span>设备</span><strong>{selectedOrder?.equipment_id ?? "未选择"}</strong></div><div><span className="repair-summary-icon">险</span><span>故障</span><strong>{selectedOrder?.symptom ?? "未选择"}</strong></div></div><div className="repair-progress"><span style={{ width: repair ? "62%" : "0%" }} /></div><p className="muted small">{repair ? "已完成现场排查和压力复测。" : "请选择已分配工单开始维修。"}</p></aside>
      <section className="repair-workspace" aria-label="维修诊断">
      ...
      <section className="data-card repair-record-card" aria-label="维修记录填写"><header className="section-heading"><div><h3>维修记录填写</h3><p>填写实际原因、处理动作与复测结果。</p></div><span className="status-chip status-chip--warning">{completed ? "已提交验收" : repair ? "待提交验收" : "等待开始"}</span></header><fieldset disabled={!repair || resultSubmitting}><div className="repair-form-grid"><label>实际原因<select aria-label="实际原因" value={result.actual_cause} onChange={(event) => setResult({ ...result, actual_cause: event.target.value })}><option value="">请选择</option><option>液压油滤芯堵塞</option><option>压力传感器异常</option><option>阀组磨损</option></select></label><label>维修方案<select aria-label="维修方案" value={result.actual_solution} onChange={(event) => setResult({ ...result, actual_solution: event.target.value })}><option value="">请选择</option><option>更换滤芯并清洗油路</option><option>更换传感器并校准</option></select></label><label>备件成本<input aria-label="备件成本" inputMode="decimal" placeholder="例如：680" value={partsCost} onChange={(event) => setPartsCost(event.target.value)} /></label><label>工时<input aria-label="工时" inputMode="decimal" placeholder="例如：3.5" value={workHours} onChange={(event) => setWorkHours(event.target.value)} /></label><label className="repair-form-grid__full">现场处理记录<textarea aria-label="现场处理记录" placeholder="填写实际检查过程、处理动作、复测结果。" value={onsiteNotes} onChange={(event) => { setOnsiteNotes(event.target.value); setResult({ ...result, repair_result: event.target.value }); }} /></label><label className="repair-form-grid__full">维修结果<textarea aria-label="维修结果" value={result.repair_result} onChange={(event) => setResult({ ...result, repair_result: event.target.value })} /></label><label className="repair-form-grid__full">备件更换说明<textarea aria-label="备件更换说明" value={result.parts_replacement_notes} onChange={(event) => setResult({ ...result, parts_replacement_notes: event.target.value })} /></label><label className="repair-form-grid__full">维修附件<input aria-label="维修附件" type="file" multiple onChange={(event) => setRepairAttachmentNames(Array.from(event.target.files ?? []).map((file) => file.name))} /><span className="muted small">附件上传接口未由当前维修结果 API 提供，已保留原型入口；不会伪造附件记录。</span>{repairAttachmentNames.length > 0 && <span className="muted small">已选择：{repairAttachmentNames.join("、")}</span>}</label></div></fieldset><div className="form-actions"><button type="button" className="button-primary" aria-label="提交维修结果" disabled={!canClose || !repair || resultSubmitting} onClick={() => void submitResult()}>{resultSubmitting ? "提交中…" : "提交验收"}</button><button type="button" className="button-secondary" disabled title="当前 API 未提供草稿保存接口">保存草稿</button></div></section>
      ...
      <aside className="data-card repair-guidance-card" role="region" aria-label="建议与引用"><header className="section-heading"><div><h3>建议与引用</h3><p>基于正式知识检索与维修上下文。</p></div><span className="status-chip status-chip--neutral">知识图谱</span></header><section className="agent-chat" aria-label="操作指引"><h3>操作指引</h3>
