...
function GlobalAgentDrawer({ onClose }: { onClose: () => void }) {
  ...
  return <aside className="agent-drawer" role="dialog" aria-modal="true" aria-label="全局 Agent" tabIndex={-1}>
    ...
    {tab === "compose" && <><div className="agent-quick-actions"><button type="button" onClick={() => setAgentId("fault_reporting")}>故障上报</button><button type="button" onClick={() => setAgentId("metric_query")}>查询设备故障</button><button type="button" onClick={() => setAgentId("operation_guidance")}>生成维修建议</button></div><section className="agent-progress"><div className="panel-heading"><h3>上报预收集</h3><span>60%</span></div><div className="progress-bar"><span style={{ width: "60%" }} /></div><p>当前页面上下文已加载，正式提交需进入 Agent 上报页。</p></section></>}
