...
describe("TASK-012 portal pages", () => {
  ...
  it("matches the prototype RBAC role workspace structure", async () => {
    vi.stubGlobal("fetch", vi.fn()
      ...
      .mockResolvedValueOnce(new Response(JSON.stringify([
        { id: "role-admin", code: "ADMIN", name: "系统管理员", permission_codes: ["system:user"], built_in: true, enabled: true, user_count: 1, updated_at: "2026-08-13T00:00:00Z", description: null },
        { id: "role-device", code: "DEVICE_ADMIN", name: "设备管理员", permission_codes: ["equipment:view"], built_in: false, enabled: true, user_count: 0, updated_at: "2026-08-13T00:00:00Z", description: null },
      ]), { status: 200 }))
      .mockResolvedValueOnce(new Response(JSON.stringify([]), { status: 200 })));
  ...
  it("lets an administrator select formal permission codes before saving a role", async () => {
    ...
    expect(JSON.parse(String((fetchMock.mock.calls[4][1] as RequestInit).body))).toMatchObject({
      code: "LINE_OPERATOR", permission_codes: ["workbench:view", "fault:create"],
    });
  });

  it("creates a custom role from the prototype editor with the formal API", async () => {
    const fetchMock = vi.fn()
      .mockResolvedValueOnce(new Response(JSON.stringify({ items: [], count: 0, page: 1, page_size: 20 }), { status: 200 }))
      ...
      .mockResolvedValueOnce(new Response(JSON.stringify({ id: "role-2", code: "PRODUCTION_SCHEDULER", name: "生产调度员", permission_codes: ["identity:write"] }), { status: 201 }));
    vi.stubGlobal("fetch", fetchMock);
    ...
    fireEvent.change(screen.getByPlaceholderText("例如：生产调度员"), { target: { value: "生产调度员" } });
    fireEvent.click(screen.getByRole("checkbox", { name: "identity:write" }));
    await waitFor(() => expect(screen.getByRole("checkbox", { name: "identity:write" })).toBeChecked());
    fireEvent.click(screen.getByRole("button", { name: "保存角色" }));
    await waitFor(() => expect(fetchMock).toHaveBeenCalledWith("/api/roles", expect.objectContaining({ method: "POST" })));
  });

  it("keeps the prototype user table and binds the formal profile fields", async () => {
    vi.stubGlobal("fetch", vi.fn()
      .mockResolvedValueOnce(new Response(JSON.stringify({ items: [], count: 0, page: 1, page_size: 20 }), { status: 200 }))
  ...
  it("keeps prototype login and operation log filters, seven-column tables and controlled field gaps", async () => {
    ...
    vi.stubGlobal("fetch", vi.fn()
      ...
      .mockResolvedValueOnce(new Response(JSON.stringify([]), { status: 200 }))
      .mockResolvedValueOnce(new Response(JSON.stringify([]), { status: 200 }))
      .mockResolvedValueOnce(new Response(JSON.stringify({ items: [{ id: "login-1", username: "admin", display_name: "管理员", logged_at: "2026-08-13T09:00:00Z", source_ip: null, client_summary: null, result: "success", reason: null }], count: 1, page: 1, page_size: 20 }), { status: 200 })));

    render(<MemoryRouter><SystemManagementPage /></MemoryRouter>);
    fireEvent.click(await screen.findByRole("tab", { name: "登录日志" }));
    await screen.findByRole("columnheader", { name: "登录账号" });
