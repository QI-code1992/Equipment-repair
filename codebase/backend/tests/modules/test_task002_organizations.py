...
def equipment_writer_headers(client: TestClient) -> dict[str, str]:
    _, token = create_user_token(
        client,
...
def test_update_rejects_type_and_parent_id(client: TestClient) -> None:
    ...
    for field, value in (("type", "WORKSHOP"), ("parent_id", factory["id"])):
        ...
        assert set(response.json()["detail"]["fields"]) == {field}
...
def test_organization_create_and_update_lock_tree_before_idempotency(
    ...
    monkeypatch.setattr(
        ...
        lambda db: calls.append("tree"),
    )
    monkeypatch.setattr(
        organization_router,
        "find_idempotent_response",
        lambda *args, **kwargs: calls.append("idempotency") or (200, {"replay": True}),
    )

    created = client.post(
        "/api/organizations",
        headers={**headers, "Idempotency-Key": "lock-create"},
        json=organization_payload(
    ...
    calls.clear()
    updated = client.patch(
        "/api/organizations/any-id",
        headers={**headers, "Idempotency-Key": "lock-update"},
        json={
            "code": "FAC-LOCK",
            "name": "锁工厂",
            "sort_order": 0,
...
def test_organization_delete_locks_tree_before_business_read(
    client: TestClient, monkeypatch
) -> None:
    headers = organization_writer_headers(client)
    calls: list[str] = []
    ...
    monkeypatch.setattr(
        ...
        "delete_organization",
        lambda db, organization_id: calls.append("business") or None,
    )
    monkeypatch.setattr(
        organization_router,
        "_complete_write",
        lambda *args, **kwargs: {"deleted": True},
    )

    response = client.delete("/api/organizations/any-id", headers=headers)
    assert response.status_code == 200
    assert calls == ["tree", "business"]


def test_equipment_writes_lock_tree_before_idempotency(
    client: TestClient, monkeypatch
) -> None:
    headers = equipment_writer_headers(client)
    calls: list[str] = []
    monkeypatch.setattr(
        equipment_router,
        "acquire_organization_tree_lock",
