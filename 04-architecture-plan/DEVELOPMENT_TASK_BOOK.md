...
- v1.5 生效条件：本修订的 PR 同时包含 Stage 5 设计、计划与缺陷记录，不适用纯治理 PR 豁免；须由项目负责人确认治理内容与精确 HEAD、DEV-001 审核精确 HEAD，并完成集成检查和单独 Merge 授权后生效；在此之前不得开始 `TASK-012` 代码实现。
...
- 状态：Completed / PR #20 代码已合入、PR #25 合并后治理收尾已合入；未进入 Stage 6
- 优先级：P0
- 负责人：`DEV-001`
- 任务开发者：`DEV-001`
- 指定审核者：`DEV-002`
...
- 交接：旧恢复点 `0b0d9cf0dc066143c0a57d4683567fadb4714c12`、证据提交 `9f162b421f4fefae4cdd69a001891c7e83d4bc13` 和被拒候选 `cfb8ed9b99b5e440b3c0bf4a8652f4f7d233ee77` 仅保留历史。R8—R11 审计脱敏修复候选均保留可追溯；Python 3.13 为 `142 passed, 5 skipped, 1 warning`，内部网络 PostgreSQL 17 为 `5 passed, 1 warning`，Compose 容器健康且 `/healthz` 为 HTTP 200。未知字段默认脱敏未纳入范围，残余风险已记录。PR #25 Merge Commit `028da42eb9ab4b55ef981ac462e09993a31e8813` 已解除 TASK-002 对下游的前置限制；Stage 6 门禁不受影响。
- PR：[#15](https://github.com/QI-code1992/Equipment-repair/pull/15) 已违规合并后由 CR-038 回滚并保留历史；DEV-002 创建的后继正式 PR #20 已合入；纯治理 PR #25 已由 DEV-002（`QI-code1992`）手动 Merge Commit 合入。
...
|---|---|---|---|
| TASK-012-API-001 | 当前用户待办、告警摘要与快捷事项读取（Workbench） | 项目负责人确认资源、权限、排序/筛选、空/错语义 | 正式只读 API 有权限/空/错契约；前端只显示真实返回结果；无前端推导或样例事项。 |
| TASK-012-API-002 | BI 摘要、趋势、效率、组织排行、历史对比与全局筛选 | 项目负责人确认指标口径、时间窗、组织维度、聚合来源、上限和权限 | API 规格、后端聚合测试与前端筛选/空/错测试一致；不得由浏览器拼接虚假排行。 |
| TASK-012-API-003 | 设备维修历史与趋势读取 | 项目负责人确认数据来源、字段、分页、脱敏、权限 | 设备详情 API 按设备返回受控历史/趋势；404/空/权限测试通过。 |
| TASK-012-API-004 | 维修记录列表、详情与知识状态读取 | 项目负责人确认查询模型、筛选、分页、知识状态和角色权限 | 记录列表/详情仅返回授权字段；空/错/权限和知识状态回归通过。 |
| TASK-012-API-005 | 分配工单列表与工单详情读取 | 项目负责人确认分配语义、维修人员可见范围、状态/排序和读取权限 | 维修执行页能以真实工单进入、读取并完成；越权和状态冲突测试通过。 |
...
## 前端协作回退基线与页面任务（CR-052，2026-08-07）
...
- `FE-002`：TEST-001 开发、TEST-002 审核。独占 `PortalPages.tsx` 与其测试，负责 BI、工厂建模、设备、维修记录、系统管理和 Agent 上报页面；`/intelligence-audit` 没有独立 Stage 3 原型，不属于“一比一原型还原”范围；`/maintenance-records/:id` 的原型依据是 `maintenance-records.html` 内的详情模态框，而非独立页面，页面整改应还原该模态框交互。若必须保留独立详情路由，或修改智能审计，均须先单独确认范围。
