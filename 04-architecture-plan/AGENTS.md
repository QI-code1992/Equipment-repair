...
本文件仅约束 Stage 5 开发实现及从后续阶段回流的 Stage 5 修复。编码前必须阅读并遵守本文件。Stage 6 独立测试、测试结论和纯治理 PR 不默认继承本文件的开发者互审、单 Draft PR 或 Merge 执行者分离规则；只有缺陷需要修改业务代码、测试代码、数据库、基础设施、部署或运行时配置时，才回流 Stage 5 并重新适用本文件。
...
本节仅适用于 Stage 5 开发任务 PR 和由后续阶段缺陷回流的 Stage 5 修复 PR。Stage 6 独立测试不要求 DEV-001/DEV-002 交叉审核；若发现需修改业务代码、测试代码、数据库、基础设施、部署或运行时配置的缺陷，必须先登记并回流 Stage 5，使用本节及当前任务书完成修复、审核和集成后，再恢复 Stage 6 独立测试。Stage 6 纯治理 PR 仅限测试/治理材料，依项目负责人对精确 HEAD 的确认、适用集成检查和单独 Merge 授权处理；除非已批准的 Stage 6 规则另有要求，不强制 DEV-001/DEV-002 互审或非作者执行 Merge。
...
- Stage 5 开发任务 PR 审核通过后，DEV-001 负责核查目标分支、当前 HEAD、审核结论、required checks、依赖、冲突、共享契约、证据、风险和回滚。
- Stage 5 纯治理 PR 仅修改流程、任务书、AGENTS 或工作流台账且不含业务代码、测试代码、数据库、基础设施或部署配置，不要求 DEV-001/DEV-002 交叉代码审核；由项目负责人确认治理内容和精确 HEAD，DEV-001 执行集成核查、Merge 授权请求和合并后治理验证，非 PR 作者的开发者在获批后执行 Merge。
- Stage 5 的 DEV-001 集成检查通过后，必须向项目负责人逐 PR 请求 Merge 授权；请求必须绑定 PR 编号与精确 HEAD。未获明确批准不得合并。
- Stage 5 项目负责人授权后，若 HEAD、目标分支、依赖或检查结论变化，授权立即失效。DEV-001 必须重新核查并重新询问；开发任务 PR 的 HEAD 变化还必须重新审核。
- Stage 5 Merge 执行者必须与 PR 的任务开发者或治理 PR 作者不同：DEV-002 开发的任务由 DEV-001 合并，DEV-001 开发的任务由 DEV-002 合并；Stage 5 纯治理 PR 由非 PR 作者的开发者合并。禁止 GitHub auto-merge、merge queue、自合并，以及直接 push 集成分支。
