...
| 知识检索 | 本地部署 RAGFlow | 负责解析、切片、向量化、检索、重排和引用。 |
...
不采用 MySQL：本期需要的 JSONB 查询、复杂聚合、窗口分析、审计关联和 LangGraph checkpoint 与 PostgreSQL 的契合度更高。也不采用微服务优先方案：在单机环境中会增加发布、日志、网络和事务协调成本，收益不足。
...
容器网络分为 `edge`、`app`、`ragflow` 三层。仅 Nginx 的 HTTPS 端口对公网暴露；PostgreSQL、Redis、MinIO、RAGFlow 和 Worker 不提供公网端口。RAGFlow 使用独立的 PostgreSQL、Redis、对象存储实例或账号，不与业务系统共享依赖账户。
...
| RAGFlow 与业务事实混淆 | 文档仅作引用证据；业务事实仅来自 PostgreSQL/API。 |
