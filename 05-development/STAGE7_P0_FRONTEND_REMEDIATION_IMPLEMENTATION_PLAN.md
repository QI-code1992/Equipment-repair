...
1. 在一个纯治理 Draft PR 中更新 `04-architecture-plan/DEVELOPMENT_TASK_BOOK.md`：新增本修复工作包、开发者/审核者/合并矩阵、依赖、回滚边界和 `DEF-STAGE7-001` 引用；同步 `workflow/state.json`、`workflow/PM_TO_DEV_HANDOFF.md`、`workflow/DEV_TO_PM_HANDOFF.md` 的“Stage 7 被阻断、回流 Stage 5”状态。
...
5. 以现有 `/api/agent-runtime/threads`、`/messages`、`/runs/{id}/events`、`/resume`、`/threads/{id}` 契约实现 `GlobalAgentDrawer.tsx`，覆盖关闭/打开、收集、预览、错误状态；SSE 使用现有受认证客户端，不能展示原始思维链。
...
2. 若任务 0 证明现有健康分、指标目录/批量查询接口足以满足某项展示，直接通过 `api.ts` 接入；若待办/告警、趋势、效率、排行或历史对比没有已批准 API，则保留页面的明确“数据服务未配置”错误状态并将对应 P1 契约缺口交由 DEV-001，不伪造卡片、曲线或排行。
...
2. 用现有组织接口、设备接口、`/api/auth/users`、`/roles`、`/permissions` 等正式契约实现以下页面：
...
4. 对设备历史、审计列表等尚无读取契约的原型区域，按任务 0 的缺口结果走 API 审批；没有批准前不加入假历史表。
...
4. 工单分配/列表/详情、维修记录/知识状态等如缺少读取或写入 API，先作为 P1 契约缺口，暂停对应交互，不将原型样例写入代码。
...
2. 仅消费现有 `/api/intelligence/*`、`/api/knowledge/documents*`、指标与 Agent 配置契约；无法读取的调用记录/Token 使用/指标项必须登记为 P1 缺口，不能展示静态仪表盘数值。
...
4. 更新 `06-testing/TEST_CASES.md`、`06-testing/TEST_REPORT.md`、`06-testing/DEFECTS.md`、`05-development/SELF_TEST.md`、`05-development/CHECKPOINTS.md` 与交接台账，绑定实际候选 SHA、缺口、未验证项、回退方式。只有所有 P0 行均有可复核证据，才可关闭 `DEF-STAGE7-001`。
5. 验证：
   ```bash
...
1. 任务 0 为纯治理 PR；必须先获得项目负责人对精确 HEAD 的治理确认、集成检查和单独 Merge 授权。
2. 任务 1 至任务 5 是 Stage 5 开发任务。建议每个任务各自一个 Draft PR，保持同一 PR 的修订与复审，避免把不同页面域混成不可回退的大型变更；PR 的任务编号、负责人和依赖以任务 0 合入后的任务书为准。
