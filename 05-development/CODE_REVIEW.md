...
- Follow-up on exact HEAD: the disposable worktree has no installed frontend dependencies (`npm run build` fails because `tsc` is unavailable), no generated `codebase/frontend/dist`, and no TLS certificate pair under `codebase/infra/nginx`. A designated validation setup must install dependencies/build the frontend and inject temporary certificates outside Git before HTTPS/browser evidence can be collected.
