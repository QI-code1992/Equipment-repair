...
- 初始实现提交：`33d7712334044437eba0d3fc884859d48a3c71ed`、`7cbf76bb9ae627e023cbeaa86fd883b18a916373`；`f7da3393f8861e3f7b8a453629fce7079915e58e` 是初始 API 实现提交。当前安全实现 HEAD 为 `04e651c1453fbd0551303aff9f4d6236ea2e59d4`，分支 `codex/task-006-agent-config`。
- 远端证据与恢复点：`2a7ca4eeec9857f361039736ec2576658832df52`。`56a3c1028ee0731d6b6ec4bc51e7dd6d73679608` 是首个证据提交，不再作为当前恢复点；证据提交与当前安全实现 HEAD `04e651c1453fbd0551303aff9f4d6236ea2e59d4` 的角色不得混同。
...
- 安全加固补充：`04e651c1453fbd0551303aff9f4d6236ea2e59d4` 已关闭最终审查的请求校验 Important；FastAPI/Pydantic 前置错误统一脱敏为稳定 `AGENT_CONFIG_INVALID` 字段响应。补充验证为 API `7 passed`、模块 `24 passed`、后端全量 `28 passed`，均仅有既有第三方 warning；原 `21/25` 证据继续保留。
- 非阻塞关注：既有四项 Minor 测试增强及 `_validate` 约 50 行长度关注仍未处理且不阻塞本检查点。路由仍未挂载；数据库状态无需回滚，数据库实现继续由 TASK-002 后续处理。
