...
### CR-052：建立前端协作回退基线并重启页面整改

- 级别：L2 前端协作、原型一致性与部署回退基线。
- 状态：Approved。
- 提出人：项目负责人。
- 提出时间：2026-08-07。
- 范围：以 `f61348d5613ed8b7221eee4ea9d3207c65db414a` 为唯一前端协作基线，建立 GitHub 标签 `baseline/frontend-collaboration-f61348d`，并按 `05-development/前端协作基线与交接.md` 划分 DEV-001 与 DEV-002 的页面和文件所有权。
- 决定：不继承后续造成页面偏差的试验性前端提交；两名开发者仅从该基线创建独立 worktree 和任务分支。
- 边界：不修改业务 API、数据库、Compose、RAGFlow、部署拓扑或 Stage 6/7/8 结论；涉及这些范围必须另行登记。
- 验证：GitHub 标签存在；ECS `SOURCE_COMMIT` 与 `deployed-sha` 均为基线 Commit；HTTPS `/healthz` 返回 200。
...
- 级别：L3
- 状态：Approved / 当前设计确认不实现 EquipmentGrant 或设备行级授权隔离
- 提出人：DEV-002（由 DEV-001 第四轮复审反馈触发）
- 提出时间：2026-07-24
- 当前阶段：Stage 5 — 开发实施
- 原始请求：为满足 AC-037，诊断与操作指引不能只依赖 `intelligence:agent`、`fault:repair` 等角色权限访问任意已知设备或故障；需先明确设备对象级授权契约，或复用现有获批对象授权机制后再修复。
...
### CR-046：Stage 7 隔离验收环境部署与项目负责人实测验收

- 级别：L2 验收基线补充。
- 状态：方案待确认 / 未执行。
- 提出人：项目负责人。
- 提出时间：2026-07-29。
- 当前阶段：Stage 7 — Product Acceptance。
- 原始请求：先进行测试部署，由项目负责人根据系统实际表现决定能否验收。
- 明确需求：先形成对已获 Gate 批准验收候选 `89fbd2129169fb6ece42094b17907885637f3c48` 的隔离部署方案；项目负责人确认方案后才可部署。部署完成后，项目负责人使用测试账号和测试数据实际操作，再签发最终验收结论。
- 原因：Stage 6 运行验证不替代项目负责人对真实产品体验和验收标准的独立判断。
- 影响：
  - PRD / SPEC / 原型 / 架构 / API / 数据模型：不变。
  - Stage 7：新增隔离验收环境的执行、证据与清理要求；逐项 AC 验收仍待实际执行。
...
- 原始请求：当前正式前端未按已批准原型实现；要求修复页面功能矩阵中的全部 P0 页面。
- 明确需求：正式 React 前端以 Stage 3 原型的视觉布局、信息架构、组件层级和关键交互为基线还原全部 P0 页面，并以真实 API、权限与加载/空/错状态替代原型演示数据；不得复制原型静态源码或以 mock 数据伪装业务能力。
- 原因：验收环境中的当前工作台仅呈现设备 ID 健康分查询表单，且正式前端只覆盖四个业务路由，不能满足已批准原型的 P0 页面范围。
- 影响：
  - PRD / SPEC / 页面功能矩阵 / 原型 / AC：不变；本 CR 纠正实现与既有基线的偏离。
  - 正式前端：新增完整 P0 路由、共享应用壳、页面、状态、真实 API 消费与回归测试。
  - 后端：先完成 API 对照；缺少或不足的契约须作为 Stage 5 子任务处理，不得用假数据替代。
  - API 完成边界：`API-GAP-001`—`007` 必须作为有负责人、依赖和验收标准的 Stage 5 前置子任务关闭；开发期或运行期“数据不可用”状态不能替代受影响 P0 功能、TASK-012 或 `DEF-STAGE7-001` 的完成。
...
- Migration: `0010_system_mgmt`. ECS automatic deployment must reject this revision. Before cutover, take a database backup, run `alembic upgrade head` manually, execute permission/idempotency/audit smoke tests and the HTTPS health check, then update the deployed SHA.
