...
### CR-053：收敛原型路由、账户安全与 Global Agent 范围

- 级别：L2
- 状态：Approved / Ready for baseline update
- 提出人：项目负责人
- 提出时间：2026-08-13
- 当前阶段：Stage 4/Stage 5 前端基线整改
- 原始请求：确认工作台正式路由、正式找回密码、修改密码安全规则、维修详情形态、智能审计范围及 Global Agent 权限范围。
- 决策：
  1. 工作台正式路由为 `/workbench`；根路径 `/` 仅作为登录后的默认跳转入口，不作为正式页面契约。
  2. 实现正式找回密码流程，包含请求找回、一次性凭证/链接、过期与已使用处理、成功后重新登录；不得在日志、URL 持久状态或响应中泄露密码或凭证。
  3. 修改密码字段为当前密码、新密码、确认新密码；服务端校验当前密码、密码策略、两次新密码一致性和账号状态；成功后使该账号现有会话失效、写入审计事件，并要求重新登录。失败不得改变会话。
  4. 维修详情统一为 `maintenance-records.html` 内详情模态框，不保留独立详情页面视觉范围。
  5. 取消独立智能审计页面；调用记录和知识文档状态统一归入智能配置中心。现有 `/intelligence-audit` 不再属于当前页面整改范围。
  6. Global Agent 对所有已登录用户可用，暂不按权限码隐藏入口或阻止调用；故障正式提交、知识重试等业务写操作仍按各自业务权限校验。
- 影响：PRD、SPEC、AC、页面功能矩阵、API 契约、前端协作任务书和原型一致性审计；涉及认证、密码和会话安全的实现需单独测试。
- 不包含：新增独立智能审计页面、设备/维修详情新视觉、绕过业务写权限、生产部署变更。
- 实现状态：待更新产品基线与 API/任务书；业务代码尚未修改。
