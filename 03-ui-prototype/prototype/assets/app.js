...
function initUserEntry() {
  ...
  menu.addEventListener("click", event => {
    ...
    if (action === "logout") { closeMenu(); if (window.confirm("确认退出当前账号？")) { showToast("已安全退出登录"); window.setTimeout(() => { window.location.href = "login.html"; }, 180); } }
  });
...
function initNotificationCenterForBell(bell) {
  ...
  const render = () => { const list = $("[data-notification-list]", panel); const items = notifications.filter((item) => filter === "all" || item.unread); if (!items.length) { list.innerHTML = `<div class="notification-empty">${filter === "unread" ? "暂无未读消息" : "暂无消息通知"}</div>`; return; } list.innerHTML = items.map((item) => `<button type="button" class="notification-item ${item.unread ? "is-unread" : "is-read"}" data-notification-id="${item.id}"><span class="notification-dot ${item.unread ? "is-unread" : "is-read"}" aria-hidden="true"></span><span class="notification-copy"><strong><em class="notification-type ${item.tone}">${item.type}</em>${item.title}</strong><span>${item.summary}</span><small>${item.object} · ${item.time}</small></span></button>`).join(""); };
